...
<!-- precedent-meta
{
  ...
  "fix": "Run4 засчитан как попытка 4 из 5 без автоповтора; последняя попытка ждёт отдельного владельческого решения",
  ...
  "actionItems": [
    ...
    {"text": "Выбрать владельцем маршрут последней внешней попытки M4", "owner": "vesnin", "status": "open"}
  ],
