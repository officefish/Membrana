# Промпт: Узел как устройство: приложение на Firebat само держит канал к серверу и получает права при установке
...
> Реестр: `id` = `firebat-node-device` в [`docs/tasks/registry.json`](../tasks/registry.json).
