# Промпт: Ресёрч: математическая оценка ответственности команды
...
> Реестр: `id` = `team-accountability-metrics` в [`docs/tasks/registry.json`](../tasks/registry.json).
...
> Заполнить ДО `yarn research team-accountability-metrics`. Вопрос — конкретный и самодостаточный:
> Perplexity ответит на что угодно, включая обрывок (инцидент 2026-07-12, #402).

1. **Landscape:** какие метрики ответственности и вклада участников применяются в деловой практике 2025–2026 — RACI/DACI и их количественные расширения, метрики загрузки и незавершённой работы, ownership-метрики в инженерных командах; что из этого считается математически, а что остаётся качественным?
2. **Fit (Membrana):** для команды, где часть исполнителей — ИИ-агенты, работающие под именем роли, а след исполнителя сейчас обезличен — как измеряют загрузку и ответственность так, чтобы метрика не выродилась в счёт строк кода или числа закрытых задач?
3. **Risk:** чем оборачивается введение количественных метрик ответственности — гейминг метрики по закону Гудхарта, демотивация, ложная точность; какие компенсирующие практики применяют?
