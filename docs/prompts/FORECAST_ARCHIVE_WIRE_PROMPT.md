...
- [ ] После прогона в `tarasov.jsonl` лежат записи `class=forecast`, число названо цифрой.
      **`angelina.jsonl` не заводится — законное «нет с причиной» (B10):** её подвид `stop`,
      а остановок в обоих спринтах 01.08 ровно ноль (гейт: «остановок 0 из 4»). Записывать
      нечего; выдуманная запись солгала бы в единственном месте, где память обязана быть
      честной. Пункт DoD в первой редакции предполагал записи у обоих авторов — ошибка
      постановки, исправлена здесь, а не подгонкой прогона
