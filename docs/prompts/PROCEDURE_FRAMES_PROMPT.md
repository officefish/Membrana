...
> Заседание: **закрыто**; этот спринт — **исполнение**, не пересборка комнаты.
...
| **Первый фрейм** | `morning-wiring` @ `ritual-day` · holder ozhegov | чинить утро инлайн без адреса |
...
|------|-----|------:|------|------|------|-------|
| **F0** | `pf-f0-brief` | [#926](https://github.com/officefish/Membrana/issues/926) | S | ozhegov | Синхрон промпта/OPEN с EPIC; **ADR** на шов `pin?` vs три пина; хвост №3 (прецедент в main → `precedents[]`) | EPIC §хвосты 1, 3 |
| **F1** | `pf-f1-frames-contract` | [#927](https://github.com/officefish/Membrana/issues/927) | M | ozhegov | `frames[]` optional в `validateProcedure`; элемент `{id, holder, pin?}`; P1∧P2∧P3; живущие процедуры валидны без `frames` | m1 |
...
| **F3** | `pf-f3-morning-wiring` | [#929](https://github.com/officefish/Membrana/issues/929) | M | ozhegov | `morning-wiring` в `ritual-day`; 3 двери; гейт на старте утра | m3 |
...
| 3 | Прецедент 22.07 в `precedents[]` | **F0**: файл уже в `docs/precedents/…cold-start-autostart.md` — добавить в `ritual-day` precedents при F3, если ещё нет |
...
2. Протоколы m1→m4 по фазе  
3. Этот промпт + Issue #900  
4. `yarn neighbors` (не пересечь active соседей)  
5. Skill `membrana-containerization-master` — паттерны ортогональны, не глотать frames в кит
