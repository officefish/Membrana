# Промпт: Мастерская задач: три глагола объявлены без движков — строить или объявить declared-not-built
...
> Ожидаемый артефакт: **1 PR — три объявленных глагола мастерской получают честный статус либо движок**.
> Реестр: `id` = `tw-declared-verbs-honest-no` в [`docs/tasks/registry.json`](../tasks/registry.json).

---

## Контекст

Опись мастерской задач (docs/tasks/workshop.manifest.json) объявляет board / bookkeeping / reviewing как planned, движков нет: yarn task:tools печатает три предупреждения «движок отсутствует». Объявление живёт, носителя нет — класс, ради которого заведён зверь B9 «Проза».

**Развилка владельца (решить ДО кода):** строить три движка или объявить их отсутствующими честно. По норме зверя B10 легальное «нет» — не дефект, а бэклог; строить инструмент ради полноты таблицы — украшение.

Не трогаем: набор глаголов паттерна HOME_WORKSHOP, соседние мастерские.
