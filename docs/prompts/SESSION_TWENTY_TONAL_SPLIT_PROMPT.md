# Промпт: Двадцать опорных — из тональных: граница абсолютом и раздельные списки (опорные / негативный материал)
...
> Реестр: `id` = `session-twenty-tonal-split` в [`docs/tasks/registry.json`](../tasks/registry.json).
