# Промпт: Переназначение двадцати фреймов: у каждого явный держатель, модератор назван отдельно (Р3 ADR-0025)
...
> Реестр: `id` = `frame-holders-reassign-twenty` в [`docs/tasks/registry.json`](../tasks/registry.json).
