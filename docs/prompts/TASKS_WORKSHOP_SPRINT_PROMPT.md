...
## g0 — HARD GATE (слово владельца)

Карточка `tw-g0-v2-gate` (#1056 / DRU-404).

Вердикт M2 относит `audit` и `decompose` **вне** мастерской; паттерн `HOME_WORKSHOP`
называет их **обязательными** глаголами. До слова владельца **запрещено** писать код
v2+ и фиксировать границу глаголов в README.

Нужен вердикт владельца в одной из форм:

1. **V2 wins** — `audit`/`decompose` вне мастерской; паттерн уточнить.
2. **HOME_WORKSHOP wins** — `audit`/`decompose` входят; вердикт M2 поправить follow-up.
3. **Третий путь** — явная формулировка владельца (записать в notes карточки g0).

Исполнение g0 = дождаться слова и записать его в Issue / Linear / `archiveNotes`. Не решать.
...
- [ ] g0 закрыт с явной записью вердикта владельца
