# Промпт: product-landing: лендинг membrana.space/ (описание продукта + CTA регистрация→кабинет + загрузка клиентов)
...
> Реестр: `id` = `product-landing` в [`docs/tasks/registry.json`](../tasks/registry.json).
