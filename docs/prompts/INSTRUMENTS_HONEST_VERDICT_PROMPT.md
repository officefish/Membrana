# Промпт: Приборы выносят вердикт, которому можно верить: ложные красные, свалка auth, слепой зонд
...
> Реестр: `id` = `instruments-honest-verdict` в [`docs/tasks/registry.json`](../tasks/registry.json).
