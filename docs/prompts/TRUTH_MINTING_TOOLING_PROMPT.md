# Промпт: Тулинг графа правды: ask-check, mint, utterance

> ⚠ **РЫБА, НЕ ПРОМПТ.** Ниже незаполненный шаблон. Содержание задачи — с эпизодами,
> эскизами и границами — живёт в **Issue #642**; начинать работу оттуда, а не отсюда.
> Помечено сознательно: 18.07 пустая рыба `ALLY_SWALLOW_EDITORIAL_GATE_PROMPT.md`
> с обещающим заголовком заставила агента выдумать процедуру «формулировки Ожеговым»,
> которой в ней не было, — и партнёрам ушёл жаргон.
...
> Реестр: `id` = `truth-minting-tooling` в [`docs/tasks/registry.json`](../tasks/registry.json).
...
**GitHub Issue:** [#642](https://github.com/officefish/Membrana/issues/642)
