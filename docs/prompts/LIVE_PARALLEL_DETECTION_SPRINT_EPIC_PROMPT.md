...
| **LP5** | `lp5-journal-report-renderers` | 5 | Журнальные рендеры FFT-порог/тенденции (client + cabinet) + фикс live-дрона |
...
### LP5 — журнальные рендеры FFT/trends + фикс live-дрона

**Пакеты:** `apps/client` (сделано), `apps/cabinet` (паритет — TODO).

Проблема: LP2 пишет `fft-threshold-test/v0.2` и `trends-fft/v0.1` в журнал, но рендерились лишь
упрощённо; в кабинете — «нет совместимого рендера». Плюс регресс: результат mic-live-drone в
ручном режиме мелькал и пропадал.

**Клиент (сделано, ветка `vesnin`):**

1. `FftThresholdReportView` — на базе мик-плагина `ReportCard`: `FrameTickStrip` + `ReportMatrix` +
   строка-мета (режим · строгость · интервал · % кадров).
2. `TrendsFftReportView` — на базе sample-library: блок победителя + `TrendsScoreRanking` (топ-3) +
   `TrendsMatchDetailTable` по клику (реконструкция `TrendsDetectionResult` из журнального DTO).
   Свёрнутый вид = одна строка (общий header: ярлык-анализатор, время, статус, победитель).
3. Полные ярлыки: «FFT пороговый тест», «Анализатор тенденций FFT».
4. Фикс `micLiveDronePluginState.resetToIdleAfterStream`: больше не сбрасывает `status` → последний
   результат держится в панели до следующего окна. Manual-mode journal append подтверждён
   регресс-тестом (8/8 в `micLiveDroneAnalysisPlugin.test.ts`).

**Кабинет (TODO):** паритет рендеров в `apps/cabinet` (`CabinetLiveJournalReportCard`) — FFT-порог и
тенденции должны рендериться и на сервере. Решить: общий пакет вью-компонентов vs дублирование по
текущему паттерну кабинета. Затем деплой `cabinet.membrana.space`.

**DoD LP5:** FFT-порог и тенденции читаемо отображаются в журнале и клиента, и кабинета; результат
mic-live-drone не пропадает в ручном режиме.
