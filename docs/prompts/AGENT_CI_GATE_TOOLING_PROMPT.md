# Промпт: Агентский тулинг: pr:wait + грабли (конфликт блокирует CI, tail в фоне, short-path)

> ⚠ **РЫБА, НЕ ПРОМПТ.** Ниже незаполненный шаблон. Содержание задачи — с эпизодами,
> эскизами и границами — живёт в **Issue #643**; начинать работу оттуда, а не отсюда.
> Помечено сознательно: 18.07 пустая рыба `ALLY_SWALLOW_EDITORIAL_GATE_PROMPT.md`
> с обещающим заголовком заставила агента выдумать процедуру, которой в ней не было.
...
> Реестр: `id` = `agent-ci-gate-tooling` в [`docs/tasks/registry.json`](../tasks/registry.json).
...
**GitHub Issue:** [#643](https://github.com/officefish/Membrana/issues/643)
