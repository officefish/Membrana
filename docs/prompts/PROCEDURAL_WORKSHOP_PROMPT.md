...
- **`inspectElement`** (`--inspect <id>`): рассмотрение одной процедуры вглубь — её второе
  ...
  `frames[]` — **ждёт #900**; пока второе измерение = подграф манифеста (честная граница).

## Границы (осознанные)

- Полная полиморфная рекурсия по фреймам процедуры — за #900 (frames[]/auditPins не построены).
