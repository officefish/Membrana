# Промпт: Состояния задач опрашивать списком, не поштучно — норма мастерской + гард
...
> Ожидаемый артефакт: **1 PR — норма в контракт мастерской + гард против поштучного опроса**.
> Реестр: `id` = `tw-state-batch-norm` в [`docs/tasks/registry.json`](../tasks/registry.json).

---

## Контекст

Состояния задач опрашивались поштучно (gh issue view в цикле) и упёрлись в пятиминутный таймаут, не доехав; тот же набор через gh issue list --json отработал одним вызовом (вещдок 26.07, 13 номеров).

Норма: состояния опрашивать списком; поштучный опрос допустим только для одной карточки. Дом — контракт мастерской, рядом с validate/invariants.

Не трогаем: сам формат карточек и схему реестра.
