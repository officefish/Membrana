...
2. **Веса модели — БУНДЛ, не runtime-fetch.** Studio скачивается и работает офлайн → веса YAMNet
   бандлятся как asset пакета, не тянутся с tfhub на каждом запуске. **Флаг владельцу:** это +~4 МБ к
   размеру. Компромисс: бандл (офлайн-гарантия) принят по умолчанию; альтернатива — lazy-fetch с кэшем.
