# Промпт: Заседание: переезд реестра задач на внешний стек (Linear)

> **СТАТУС: СКЕЛЕТ, НЕ ГОТОВЫЙ ПРОМПТ.** Заготовка `task:register`; работа велась
> по повесткам заседания, не по этому файлу. Не исполнять как задачу.
...
> Реестр: `id` = `meeting-registry-relocation` в [`docs/tasks/registry.json`](../tasks/registry.json).
