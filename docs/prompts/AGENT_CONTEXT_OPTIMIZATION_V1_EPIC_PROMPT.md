...
**Ветка:** `night/agent-context-optimization-v1-2026-06-27` от `techies68`.

| Phase | Scope | Lead | Gate |
|---|---|---|---|
| NB0 | Baseline, registry, frozen scope, existing-change audit | Vesnin + Ozhegov | clean scoped baseline |
| NB1 | C1 graph smoke, C2 archive-only topK, C3 audit-reads hook | Dynin | script checks + RAG tests |
| NB2 | C4 graph-first docs, C5 Headroom exclude, C6 proxy guide | Ozhegov + Dynin | docs lint + secret hygiene |
| NB3 | Scoped CI, checkpoint log, morning handoff | Vesnin | all gates green or explicitly deferred |

**In scope:** только C1–C6 и тесты/документация, перечисленные ниже.

**Out of scope:** prod deploy/smoke, CI pipeline, Issue #185, реализация proxy-сеанса,
изменения логики или публичных контрактов `@membrana/core`.

**Stop rules:** после двух последовательных падений одного CI gate — остановка фазы и handoff
с блокером; при необходимости менять `@membrana/core`, prod или расширять scope — немедленный stop.
