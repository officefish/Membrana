...
> либо для человека-DevOps. Скопируй блок «Промпт целиком» в начало диалога
> с агентом — он содержит всё, что нужно для работы. Размер задачи: **M**.
> Ожидаемый артефакт: 1 PR с инфра-документацией и опционально автоматизацией
> деплоя + работающий публичный `https://<domain>/health` → `200`.
