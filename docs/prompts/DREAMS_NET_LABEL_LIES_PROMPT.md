# Промпт: Сны метят HTTP-404 и отсутствие ключа как сетевой отказ: подключить классификатор контейнера network
...
> Реестр: `id` = `dreams-net-label-lies` в [`docs/tasks/registry.json`](../tasks/registry.json).
...
**GitHub Issue:** [#1425](https://github.com/officefish/Membrana/issues/1425)
