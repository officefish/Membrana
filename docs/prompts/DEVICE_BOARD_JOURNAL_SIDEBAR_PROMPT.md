...
> **⚠️ Статус (BTJ, 2026-07-07):** вкладка «Журнал» этого промпта (живой хвост
> scenario-trace) **переименована в «Трейс»** спринтом `board-telemetry-journal`
> (#278). Новая вкладка **«Журнал»** показывает телеметрию устройства
> (треки/отчёты) через слот от хоста. Ресайз и UX трейса сохранены. Актуальная
> раскладка сайдбара: «Узлы | Журнал | Трейс». См.
> [`BOARD_TELEMETRY_JOURNAL_SPRINT_PROMPT.md`](./BOARD_TELEMETRY_JOURNAL_SPRINT_PROMPT.md).
