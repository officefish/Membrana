# Промпт: Боевой вход ведёт к своду сеанса: повод из закрытого словаря, отпечатки рода report, словарь сборщиков контекста
...
> Реестр: `id` = `report-entry-gap` в [`docs/tasks/registry.json`](../tasks/registry.json).
