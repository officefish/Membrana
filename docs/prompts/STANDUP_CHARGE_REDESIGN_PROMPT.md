# Промпт: DAILY_STANDUP как настоящий стендап: дисциплина+честность+код-стайл и раскрытие сильных сторон персон
...
> Реестр: `id` = `standup-charge-redesign` в [`docs/tasks/registry.json`](../tasks/registry.json).
...
> Заполнить ДО `yarn research standup-charge-redesign`. Вопрос — конкретный и самодостаточный:
> Perplexity ответит на что угодно, включая обрывок (инцидент 2026-07-12, #402).

1. **Landscape:** какие практики сложились в 2025–2026 для ежедневного стендапа в командах, где исполнители — LLM-агенты с закреплёнными ролями: что реально попадает в такой стендап, как в нём удерживают единый стиль кода и принципы честности, и по каким признакам судят, что стендап помог работе, а не стал ритуальным текстом, который никто не читает?
2. **Fit (Membrana):** как на практике подсвечивают сильные стороны отдельного участника команды и подбирают ему подходящие задачи, если о нём есть журнал прошлых решений с оценками и цитатами — какие сигналы берут из истории, чтобы назначение роли на задачу опиралось на факты, а не на привычку, и как избегают закрепления участника в узкой нише?
3. **Risk:** чем рискует команда, когда ежедневный документ пишется мотивирующим тоном и подаётся как общий настрой — ложная уверенность, вытеснение неудобных фактов, усталость от повторяющегося текста; какие приёмы применяют, чтобы такой документ оставался честным и проверяемым, а не превращался в декорацию?
