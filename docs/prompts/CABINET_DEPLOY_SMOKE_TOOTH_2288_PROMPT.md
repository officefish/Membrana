...
после `up` и до `pass` smoke обязан краснеть на старом/испорченном образе для тарифов и
сетки, а `/v1/pair` держать как транспортную пробу endpoint-формы: без `5xx` и без
транспортного `4xx`.
...
2. `POST /v1/pair` с заведомо негодным ключом и непустым JSON body — транспортная проба
   endpoint-формы. Допустим доменный отказ: `401`/`404` или `200` с `ok:false`.
   `400 Body cannot be empty`, пустой body, transport parse error и любые `5xx` —
   красный smoke. Эта проба не доказывает media-bridge дефект из #2293: invalid key
   отсекается до вызовов `mediaBridge.*`.
3. Runtime image должен нести тарифную сетку:
   `docker exec cabinet-api test -f /app/docs/tariffs/tariff-grid.json`.

Каждый пункт должен иметь порчу: тест, который показывает, что сигнал краснеет.
Вчерашние production-сигналы здесь два: `503` на tariffs и отсутствующий файл сетки в image.
Для pair нужна синтетическая порча транспортного класса (`400 Body cannot be empty` / `5xx`),
не заявка на покрытие bridge-body дефекта.
...
- [ ] Порча покрывает два вчерашних сигнала до hotfix и отдельный transport-class pair red.
...
- [ ] Invalid-key `POST /v1/pair` краснеет на transport `400 Body cannot be empty` и `5xx`.
