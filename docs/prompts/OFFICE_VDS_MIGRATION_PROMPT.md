...
> **★ ВЕРДИКТ OM4 (2026-07-12): блок был IP-specific — решён сменой IP, KZ НЕ понадобился.**
> Владелец отвязал `94.141.162.3`, привязал новый публичный `176.124.218.4` (др. подсеть) на том же мск-VDS. Проверка нового IP: ICMP OK, **SSH kex+auth прошли, 10 МБ bulk за 4.2с без стопа** → data-path чист (то, что падало на старом). **OM4-A ✅** (IP-specific подтверждён). **OM4-B/KZ → разжалован в fallback** (на случай повторной пометки IP; провижн-инструкции ниже сохранены). **OM4-C ✅**: DNS `office.mmbrn.tech`→176.124.218.4 (все 4 NS), prod LE cert (YE2, Jul 12→Oct 10), `https://office.mmbrn.tech/health` 200 извне, unsigned webhook 403, :3000 закрыт. **Осталось OM4-D** (cutover). TLS-ловушка: ретраи Caddy на устаревшем DNS сожгли LE rate-limit — не рестартовать Caddy до консистентности всех 4 authoritative NS.

**OM4 — (историческая ветка пивота на KZ; оставлена как fallback-runbook) релокация на non-RU хост:**
