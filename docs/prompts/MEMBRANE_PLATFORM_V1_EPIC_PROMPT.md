...
`background-media` даёт data-plane по анонимному `deviceId`. Нужен **личный кабинет** (`cabinet.membrana.space`), login/password, модель **Мембрана → Узел → Device**, управляемые **ключи с TTL enum** (4ч, 3д, 2н, 1мес, 3мес), тарифы с **отдельными** квотами dataset/buffer, pairing в **`apps/client`**, облачный журнал (**TelemetryReport** + **TelemetryLiveRecord**).
...
| MP3 Pairing | `membrane-platform-mp3-client-pairing` | Режим «Связь с мембраной» в `apps/client` | 3 |
| MP4 Media | `membrane-platform-mp4-media-membrane` | Media scope `membraneId`, отдельные dataset/buffer квоты | 4 |
...
- [ ] `dataset` и `buffer` квоты отображаются и enforced отдельно (tariff `free-v1`: 1 GiB + 1 GiB).
...
| Desktop | `apps/client` | Pairing flow, media client, telemetry upload |
