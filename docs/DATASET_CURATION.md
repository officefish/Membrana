...
> **Слепота корпуса (правка 15.08).** До 15.08 разметить пилот вслепую было нельзя:
> файлы назывались `pilot-drone-001` / `pilot-not-helicopter-01` и лежали в каталогах
> `drone/` и `not-drone/` — оператор видел ответ раньше, чем слышал запись, а истина
> с подсказкой не истина. Корпус переразложен глаголом `yarn vdr:blind`: один каталог
> `samples/`, нейтральные имена `pilot-NN.wav`, порядок — детерминированная
> перестановка по хешу исходного id (номер не коррелирует с классом). Провенанс жив
> в манифесте полем `provenanceId` и в `blind-map.json` — **смотреть их после
> разметки, не до**. Числа замера переименование не изменило (сверено 15.08).
