...
[`sent-log.jsonl`](sent-log.jsonl) — append-only след `telegram:swallow` и ритуальных дайджестов
после `sent=true`. Поля: `ts`, `kind` (`swallow`|`digest`), `file`, `sha256` (не тело текста),
`sent`, `office_response` (`{sent, message_id?}`).

Предикат «ушла ли ласточка»: `hasSentRecord` в `scripts/lib/comms-sent-log.mjs` или grep по
`sha256` / `file`. Не путать с `.membrana/swallow-deliveries.jsonl` (#664, gitignore, идемпотентность).
