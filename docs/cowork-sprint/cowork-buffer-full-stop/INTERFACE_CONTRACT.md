...
| Решено | `smart_cleanup` до T12 | локальный страж при `smart_cleanup` молчит (`HoldActivation = 'ignored'`, C); серверный отказ при `smart_cleanup` **записывает эпизод и даёт один сигнал окна, но шлюз не закрывает** (`isHeld = эпизод ∧ policy === 'stop'`, C) — ровно буква вердикта M3 «гасится при `stop`». **Поправка Phase 4 (07.09):** первая редакция этой строки требовала удержания при любой политике; это означало бы переписать блок C (одна строка + два зуба) сверх вердикта — координатор не стал: блок оставлен по M3, а дыра «`smart_cleanup` без алгоритма T12 = пробы стучат после отказа» названа долгом D-1 в RETROSPECTIVE (гейт выбора `smart_cleanup` до появления T12 — решение владельца) |
...
4. **B→C.** Сырой `/quota` с `bufferPolicy.mode = smart_cleanup` через `getQuotaRaw()` → мост → `getEffectiveOverflowPolicy() = 'smart_cleanup'` → локальный страж `ignored`; серверный отказ при этом записывает эпизод (сигнал окна один), шлюз открыт — по M3 (см. поправку выше, долг D-1). `/quota` упал → `stop`.
