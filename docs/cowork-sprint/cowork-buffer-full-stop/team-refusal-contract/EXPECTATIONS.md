...
## Дельта Phase 2 (по фактам сборки, 06.09)

| Что | Стало известно | Кому |
|---|---|---|
| Относительный импорт типов словаря из media | Компилируется только с атрибутом `with { 'resolution-mode': 'import' }` (TS1541 без него) — та же форма, что понадобится после переключения на имя пакета; замена на интеграции = только путь модуля | координатор |
| `verify-swagger.mjs` читает словарь | Ожидаемые литералы берутся из `packages/plugin-contracts/dist/buffer-overflow/reasons.js` — `dist` соседа обязан существовать (project reference media строит его через `tsc -b`) | координатор (CI: `media:verify-swagger` уже идёт после `yarn build`) |
| Статус ответа | `@HttpCode` не годится: `FastifyAdapter.reply()` ставит статус маршрута ПОСЛЕ хендлера. Ручка использует `@Res()` и `reply.status(200|201).send(...)` явно | C (клиент судит по телу `ok`, не по 200/201) |
| Ключ эпизода | `deviceId × subject` — у одного прибора два независимых эпизода (буфер, хранилище); удаление из одной оси эпизод другой не трогает | C, 4/4 (окно — на id, id разные на два субъекта) |
| Порчи с красным | 9 прогонов, протокол — `CHECKS.md` этого каталога | координатор |
