...
**Operator notes (инспектор узла):** реестр `graph/scenario-node-inspector-notes.ts` + `BoardNodeInspectorNotes` — статические подсказки по nodeKind (не в JSON сценария). Пример: `start-recording` — защита от частого вызова. CONCEPT §15.5.1.
