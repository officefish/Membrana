...
|-----|------|--------|------|
| [ADR-0014](./ADR-0014-worktree-base-branch-sync.md) | 2026-07-20 | DRAFT | Синхрон базовых веток рабочих деревьев с `main` (K1 заседания `worktree-hygiene-gaps`): сигнал на входе в дерево (`ritual:day` + `yarn worktree:sync`), чистый предикат после fetch (`fresh`/`ff-able`/`diverged`/`dirty`), авто только `--ff-only`, расхождение/грязь — сигнал; область из реестра K2, `main` исключён. Записан ADR т.к. комната M2 4× уронила секцию посылок (S-M2), содержание стабильно |
