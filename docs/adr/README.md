...
> **Дрейф реестра (18.07):** в каталоге лежат ADR `0002`–`0005`, которых в таблице нет
> вовсе, а номер `0005` занят дважды (`0005-night-narrative-provider-chain`,
> `0005-panel-users-store`). Бэкфилл реестра и разведение коллизии — отдельная задача.
