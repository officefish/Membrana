...
|-----|------|--------|------|
| [ADR-0028](./ADR-0028-pair-media-token-per-device.md) | 2026-08-20 | PROPOSED | Парринг не раздаёт служебный токен: per-device ключ media по образцу `NodeKey`. Наблюдаемое 20.08 кодом: `PairService.pair` возвращает клиенту `MEDIA_API_TOKEN` — служебный токен всего media-сервиса. Блок b5 спринта `studio-firebat-user-pairing`, форма — vesnin; реализация вынесена в ОТДЕЛЬНЫЙ спринт `media-per-device-token` |
| [ADR-0027](./ADR-0027-field-node-outbound-poll-channel.md) | 2026-08-19 | PROPOSED | Полевой узел держит канал к серверу сам: опрос очереди заданий, дом — media, ключ узла. Узел = физический хост в поле за чужим роутером, входящих соединений не принимает. Блок b1 спринта `firebat-node-device`, нарезка ратифицирована владельцем 19.08, форма — ozhegov |
| [ADR-0029](./ADR-0029-cabinet-registration-by-panel-code.md) | 2026-09-23 | ACCEPTED | Регистрация в кабинете по промокоду **панели**: панель — единственная правда о коде (тот же `PromoCode` + грант `cabinet-register`, дверь `/v1/internal/cabinet/registration-codes/consume` под внутренней охраной, пять причин отказа в ратифицированном порядке); офис — мост с одним выходом наружу и парой `OFFICE_URL` / `OFFICE_API_TOKEN`; кабинет — потребитель с порядком «проверки без последствий → гашение → создание» и одной фразой наружу; веб-кабинет — форма на странице входа, фраза по HTTP-статусу. Фиксирует ратифицированное заседанием `cabinet-registration-promo` (M0–M4, 21.09) и влитое в ствол 22.09 · эпик #2369 · LGTM владельца → ACCEPTED |
...
| [ADR-0015](./ADR-0015-frame-pins-array-shape.md) | 2026-07-22 | ACCEPTED · Refined-by: [ADR-0025](./ADR-0025-frame-holder-and-moderator-two-roles.md) | Шов Ф1↔Ф3: канон `pins?: Pin[]` (не скаляр `pin?`); transitional чтение `pin`→`[pin]` в F1; запись F3 только `pins`. Спринт #900 F0 · LGTM владельца 22.07. **Уточнение 08.08 ([ADR-0025](./ADR-0025-frame-holder-and-moderator-two-roles.md)):** элемент фрейма получил второе поле `moderator?`, а содержание `holder` определено (разработчик, чинит код). Форма `pins` не затронута; решение не отменено — дополнено |
