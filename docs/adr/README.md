...
|-----|------|--------|------|
| [ADR-0029](./ADR-0029-cabinet-registration-by-panel-code.md) | 2026-09-23 | DRAFT | Регистрация в кабинете по промокоду **панели**: панель — единственная правда о коде (тот же `PromoCode` + грант `cabinet-register`, дверь `/v1/internal/cabinet/registration-codes/consume` под внутренней охраной, пять причин отказа в ратифицированном порядке); офис — мост с одним выходом наружу и парой `OFFICE_URL` / `OFFICE_API_TOKEN`; кабинет — потребитель с порядком «проверки без последствий → гашение → создание» и одной фразой наружу; веб-кабинет — форма на странице входа, фраза по HTTP-статусу. Фиксирует ратифицированное заседанием `cabinet-registration-promo` (M0–M4, 21.09) и влитое в ствол 22.09 · эпик #2369 · LGTM владельца → ACCEPTED |
