...
|-----|------|--------|------|
| [ADR-0018](./ADR-0018-tests-container-selective-gate-nightly-full.md) | 2026-07-26 | DRAFT | Мердж-гейт выборочный (`smoke` + `gate`), полный набор `full` — ночной от пина на `main`; результат ночи приходит в утро через фрейм `night-report` как блокер дня; дом контейнера тестов — корневой `tests/`, оснастка — мастерская. Условие честности: отчёт «что не гонялось». Предусловие: защита `main` · эпик `tests-container` (#1291/#1292/#1293) |
