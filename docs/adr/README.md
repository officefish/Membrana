...
|-----|------|--------|------|
| [ADR-0023](./ADR-0023-deploy-node-form-one-procedure-per-echelon.md) | 2026-08-04 | DRAFT | Узел разворачивания: ОДНА процедура `deploy` (фреймы `build → rollout → smoke → record`), сервис — параметр, не копия; выкладка только через owner-gate (механика `deploy:when-green` — канон); деплой пишет прогон в общий журнал; датированным одноразовым ssh-скриптам — правило осадка; дрейф прод↔ствол — шаг smoke для всех сервисов. Грунт: обзор `deploy-survey-2026-08-03` (52 глагола + 56 ssh при нуле процедур). LGTM владельца → ACCEPTED |
