...
> **Refined-by:** [ADR-0025](./ADR-0025-frame-holder-and-moderator-two-roles.md) (08.08)
>
> Связь — отдельным полем, а не третьим статусом: канон знает два движения, `ACCEPTED` и
> `SUPERSEDED-by-ADR-MMMM`. «Уточнён» статусом не является, иначе завтра появятся
> «пересмотрен» и «дополнен», и форма поплывёт (разбор Веснина 08.08).
>
> Уточнение, не отмена: ADR-0015 задал элемент фрейма формой `{ id, holder, pins? }` и
> **не определил содержание `holder`** — валидатор проверял единственное, членство в общем
> списке персон, где модератор стоял наравне с разработчиками. ADR-0025 определил `holder`
> как разработчика, который чинит код фрейма, и ввёл второе поле `moderator?` — кто ведёт
> момент. Форма `pins` этим решением не затронута.  
> LGTM владельца 22.07 («все подтверждаю»).
...
- [x] LGTM владельца 22.07 — статус ADR → ACCEPTED
