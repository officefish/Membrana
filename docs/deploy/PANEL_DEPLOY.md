...
ВНЕ git, живёт только на VDS. Деплой-скрипт **сохраняет уже выложенный бандл**
через `rm -rf dist/*` сам — отдельно ничего делать не нужно. Залить/обновить его
из локального корпуса:

```bash
yarn detector:compare:export --audio-out /tmp/compare-audio
node scripts/_ssh-panel-deploy.mjs --audio /tmp/compare-audio
```

Ручной `scp -r` больше не нужен: он же требовал ручного `chmod` следом (см. анти-паттерны).
...
- **Доверять правам после scp с Windows** (живой урок 2026-07-14, деплой #457):
  ...
  молча отдаёт index.html вместо статики (ломаются и /assets). `_ssh-panel-deploy.mjs`
  теперь делает `chmod -R a+rX` сам; при выкладке файлов мимо скрипта — повторить
  руками: `yarn office:ssh 'chmod -R a+rX /opt/membrana-panel/dist'`.
- Сносить `dist/compare-audio` руками — бандла нет в git, восстановление только
  ре-экспортом корпуса (`--audio`); деплой-скрипт его сохраняет намеренно.
