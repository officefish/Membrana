...
[Музыкант]: По assets: 57 service PNG/SVG — engine projection material. Asset с несколькими parents не «оригинал на N страниц»; disposition через ref-list + M1 qualification. Если asset только decor engine — DISCARD_ENGINE_COPY после export evidence; если portable annotation references bytes — HOLD до parity.
...
[Архитектор]: Causal DAG: S0 authority freeze read-only facts → S1 inventory seal → S2 disposition seal → S3 provision target (не требует M4 PASS) → S4 M4 G1–G10 на target → S5 M5 export → S6 rehydrate → S7 parity → S8 M6 intents only for lawful objects → S9 stage routes/proxy → S10 canary → S11 cutover DNS/Caddy → S12 health proof → S13 observe → S14 retirement arm → S15 delete source only after all retirement gates. Pre-step проверяет только existing input/authority/backup.
...
[Музыкант]: Annotation parity mismatch — не «почти музыка»: G4/G1 fail ⇒ объект в `HOLD` + parity_fail append event, cutover global STOP если mismatch в committed-set. Disposable layout mismatch не блокирует; portable annotation mismatch блокирует.
...
| `STORAGE_READY` | M4 G1–G10 PASS on target | → `PROJECTION_EXPORTED` | M4 evidence pack | fix capacity; new G* run | STOP/NO-GO on fail |
...
| `PARITY_FAILED` | any M5 G1–G5/G4 annotation fail in scope | → `PROJECTION_EXPORTED` (new attempt) \| `ABORTED` | parity diff | new attempt id | STOP cutover |
| `PARITY_PASSED` | M5 G1–G10 evidence true | → `INTAKE_ALIGNED` | parity certificate | n/a | non-terminal |
| `INTAKE_ALIGNED` | all MIGRATE_* have M6 intents; no `1/0`; legacy HOLD | → `ROUTES_STAGED` | L_proposed diff+registry join | reconcile repair/reject append | STOP if fake bind attempt |
| `ROUTES_STAGED` | proxy matrix loaded; WS deny; no DNS flip yet | → `CANARY_RUNNING` \| `ABORTED` | route config hash | restage append | R1 abort ok |
...
|-------------|---------------|-----------------|------------------|----------------|---------------|---------------|-------------------|
| legacy UI entry | `strategy.mmbrn.tech/*` | office Affine `:3010` | none (pre) / deny unclassified | live Affine (frozen publish) | synthetic only or dual-run deny user native | `308` mapped \| `404` unmapped | restore Caddy→Affine origin |
...
| static container API | `static.mmbrn.tech/api/...` | static service | one of 8 actions; object container/collection/lineage | may be dark | canary principals | LIVE after proof | stay up cold; unpublish LIVE |
| Panel authorize | Panel session → proxy | forward-auth | action+principal+object+versions | grants exist; ingress incomplete | required PASS | required PASS | unchanged Panel |
...
| download bytes | static path | FD-1 | `download`/`read-bytes`+object | NO-GO w/o M3 | canary allowlist | fail-closed allow | revoke routes |
| manage-access | Panel only | Panel | `manage-access` | Panel | Panel | Panel | Panel |
...
| S4 M4 READY ← S3 | target exists | capacity/backup/restore drills | G1–G10 pack | Математик | NO-GO; no export |
| S5 Export ← S4 | M4 PASS; disposition sealed | export bundle | counts 82/57 reconciled + hashes | Музыкант+Математик | STOP gap |
| S6 Rehydrate ← S5 | export sealed | load projection target | rehydrate report | Музыкант | retry new export |
| S7 Parity ← S6 | rehydrate done | none / checks | M5 G1–G10 | Математик+Музыкант | PARITY_FAILED stop cutover |
| S8 M6 align ← S7 | parity PASS | intents+commits lawful only | no `1/0`; legacy HOLD | Структурщик+Математик | STOP fake bind |
| S9 Routes stage ← S8+#1303B | matrix compiled | proxy config staging | config hash; WS deny test | Структурщик | R1 abort |
| S10 Canary ← S9 | staging OK | limited traffic/synthetic | ≤1%/15m ∧ health ∧ G6 | Teamlead+Математик | abort canary |
...
| M4-G1 Capacity | B=min(P-12GiB,⌊.90P⌋); free_after≥12GiB ∧ util<.90 ∧ U_c+Δ≤Q_c; unk DENY | target volume | capacity report | **FAIL** office 9.46 GiB | NO-GO target |
...
| M4-G6 Reconcile | 0 unexplained dangling/orphan | stores | reconcile diff | **NOT PERFORMED** | NO-GO |
...
| M5-G1..G10 | exact M5 predicates agenda | export/rehydrate/replacement | certificates | **NOT PERFORMED** | NO-GO S7 |
| M6-INTAKE | C_*; no `1/0`; COMMITTED only FD-1+M2+binding | registry+ledger+FD-1 | full diff join | **NO-GO** legacy uncovered | no production intake |
