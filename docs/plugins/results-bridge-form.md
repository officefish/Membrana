...
media: onResult(manifest, ctx, result)
   ...
   → PluginResultsBridgeService.send(record)            // POST, X-Membrana-Token, таймаут
...
- **Идемпотентность — у дома.** Повтор того же `runId` упирается в уникальный индекс M3′;
  приёмник отвечает «уже есть», не 500, — повторная отправка безопасна по построению.
...
результат остаётся в логе именем) · `office-unreachable` (сеть/таймаут) · `office-rejected`
...
3. Лабораторная дорожка приёмки: оба сервиса локально, office с Mongo по туннелю (как 18.08)
   или с volatile memory store — тогда вещдок только о проводе, не о доме.
