# Active day sprint

Допускается параллель (см. [`DAY_SPRINT_REGULATION.md`](./DAY_SPRINT_REGULATION.md) § ACTIVE/LOG).

## Focus (этот сеанс / W4)

| Поле | Значение |
|------|----------|
| **Sprint** | [`bestiary-workshop-2026-07-22`](./day-sprint/bestiary-workshop-2026-07-22/OPEN.md) |
| **Epic** | `bestiary-workshop` · [#945](https://github.com/officefish/Membrana/issues/945) |
| **Phase now** | **W4** `bw-w4-trap-kit` · [#950](https://github.com/officefish/Membrana/issues/950) |
| **Next** | W5 `bw-w5-closure` · [#951](https://github.com/officefish/Membrana/issues/951) — после LGTM dynin на W4 |
| **Lead phase** | dynin |
| **Lead epic** | vesnin |
| **Opened** | 2026-07-22 |
| **Seed** | [`storm-bestiary-workshop-2026-07-22`](./storm/storm-bestiary-workshop-2026-07-22/REPORT.md) |
| **Branch (W4)** | _(указатель; код не стартован)_ |
| **W0** | [#952](https://github.com/officefish/Membrana/pull/952) · `c1f067a7` · archived |
| **W1** | [#954](https://github.com/officefish/Membrana/pull/954) · `496ecb41` · archived |
| **W2** | [#965](https://github.com/officefish/Membrana/pull/965) · `7887ad73` · archived |
| **W3** | [#967](https://github.com/officefish/Membrana/pull/967) · `ed997ce1` · archived |

Последний **закрытый:** [`procedure-frames-2026-07-22`](./day-sprint/procedure-frames-2026-07-22/CLOSURE.md) (#900) · до него [`bestiary-container-2026-07-21`](./day-sprint/bestiary-container-2026-07-21/CLOSURE.md) (#878).
