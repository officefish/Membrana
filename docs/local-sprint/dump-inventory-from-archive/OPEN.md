...
## Ревью-цикл после закрытия прогона (снаружи формата, прецедент s-queue-tail)

Ревью ветки дало 10 находок; 8 вычинены тем же днём с зубами (скоуп дампа `--db` — три
скоупа сведены к одному имени; пре-клин цели до `up`; видимый `partial--` след при стопе
описи вместо невидимого `.in-progress`; `unhealthy`-баг `waitHealthy` (подстрока → точное
слово); своя цель `archivarius-dump-inventory` вместо цели дрилла; listing-only без оплаты
sha256-канонизации; дубль валидации v2 и ложная v1-жалоба на неизвестной версии; операторские
доки v2 — рунбук и README стенда). Живой прогон повторён на фиксах: манифест
`e964400d…`, дрилл зелёный. Два долга рождены мостику: `archive-restore-buffers-whole-file`
(стриминг вместо readFileSync, латентный потолок ~2 ГиБ) и
`drill-reexport-bridge-outlives-sprint` (срок жизни моста — требование держателя b2,
пойманное ревью). Ревью-фиксы легли вне зон закрытого плана — это внешний цикл PR, не
исполнение блоков; тексты решений — в этом разделе и в диффе.
