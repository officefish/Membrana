...
3. **Авторизация «какой токен за какой плагин пишет».** Вне предмета собрания (M5′, A5-5:
   границы авторизации); один ключ класса X-Membrana-Token на сервис. Владелец — Архитектор,
   момент — вторая волна плагинов / внешние писатели; до тех пор писатель один (media).
4. **Отпечатки при `request`.** `configHash` плагино-специфичен; вход считает их тем же
