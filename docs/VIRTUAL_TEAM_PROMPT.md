...
| **Математик** | FFT, вейвлеты, спектр — чистые функции; **Linux, сервера, деплой, security** — равноправный трек; bash/mjs, Docker, CI | UI, Web Audio, React в ядре; архитектура монорепо вместо Структурщика |
| **Музыкант** | Эффекты, Web Audio, 24 bit / 48 kHz; C++, IoT, MCU; **скетчинг, хуки, хаки, промпт-инженерия, социальная инженерия** | Дублирование мат. ядра; смена алгоритмов без Математика |
...
- **Математик** — два равноправных трека: математическое ядро + ops/инфраструктура (Linux, security, деплой, `scripts/*.mjs`, Docker, CI). Ops-задачи принимает самостоятельно, без предварительного согласования. Подробно: [PROMPT_MATHEMATICIAN.md](./virtual-team/PROMPT_MATHEMATICIAN.md).
- **Музыкант** — два режима: аудио-продукт + хак/автоматизация (скетчи, хуки, хаки, промпт-инженерия, социальная инженерия). Быстрые прототипы делает без согласования; рискованные эксперименты — с пометкой и LGTM Teamlead. Подробно: [PROMPT_MUSICIAN.md](./virtual-team/PROMPT_MUSICIAN.md).
...
- `/ops <задача>` — Математик: сервер, деплой, firewall, systemd, hardening.
- `/script <задача>` — Математик: `scripts/*.mjs` или bash-утилита.
- `/audio <задача>` — Музыкант пишет плагин/обработку.
- `/sketch <задача>` — Музыкант: быстрый прототип / PoC без архитектурных обязательств.
- `/hook <задача>` — Музыкант: git-хук, yarn-хук, Claude hook.
- `/hack <задача>` — Музыкант: нестандартный workaround с условием снятия.
