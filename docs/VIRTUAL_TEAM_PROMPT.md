...
| **Структурщик** | **Ozhegov** (С. И. Ожегов, 1900–1964; лингвист, лексикограф, «Толковый словарь») | `ozhegov` | `ozhegov` | [аватар](https://i.pravatar.cc/400?u=membrana-ozhegov-1900) | [PROMPT_STRUCTURER.md](./virtual-team/PROMPT_STRUCTURER.md) |
...
<https://i.pravatar.cc/400?u=membrana-vesnin-1923> · <https://i.pravatar.cc/400?u=membrana-ozhegov-1900> · <https://i.pravatar.cc/400?u=membrana-dynin-math> · <https://i.pravatar.cc/400?u=membrana-musician> · <https://i.pravatar.cc/400?u=membrana-layout-dev>
...
# Структурщик (реестр, сторы, связанность)
yarn ask ozhegov --gh-issue 11 --save-as MEM-88-registry-tests "как разбить тесты agenda без дублирования?"

# Когда задача — отдельный markdown-файл
yarn ask vesnin --ticket-file ./ticket.md "сформулируй кратко границы"

# Свободный вопрос: по умолчанию пишется в docs/discussions с авто-id (см. stderr после ответа)
yarn ask vesnin --no-context "одной фразой: нужен ли ADR сейчас?"

# Продолжить тот же тред (бот прочитает предыдущие вопросы и ответы из файла)
yarn ask vesnin --discussion TEC-42-fft "уточни про тесты на границах окна"
```

Что подкладывается в контекст:

1. Системный промпт персонажа из [`virtual-team/PROMPT_*.md`](./virtual-team/).
2. Стратегический контекст из [`WHITE_PAPER.md`](./WHITE_PAPER.md) (если не `--no-context`).
3. Выдержки из [`ARCHITECTURE.md`](./ARCHITECTURE.md) и [`SERVICES.md`](./SERVICES.md) (если не `--no-context`).
4. Контекст задачи: GitHub Issue (`--gh-issue`), markdown-файл (`--ticket-file`) или строка (`--task`).
5. Если уже есть файл треда `docs/discussions/<id>.md` — полный текст файла как **история переписки** (до лимита скрипта).

Сохранение обсуждения:

- По умолчанию **каждый** вызов дописывает ответ в `docs/discussions/<id>.md`, кроме явного `--no-save`.
- Явный id треда: `--discussion <id>` или синоним `--save-as <id>` (удобно под Linear-ID).
- Без явного id: если заданы `--gh-issue` / `--ticket-file`, имя файла как раньше (`gh-issue-N` или basename тикета); иначе генерируется авто-id вида `discussion-YYYYMMDD-HHMMSS-<hex>` — его же передайте в следующем вызове как `--discussion …`, чтобы модель видела историю.
- Если файл треда уже существует, перед запросом он **читается целиком** в промпт (память треда).
- Черновик без файла и без истории: `--no-save`.
...
Текущие персонажи: `vesnin`, `ozhegov`, `dynin`. Расширение — через `scripts/ask-persona.mjs`
