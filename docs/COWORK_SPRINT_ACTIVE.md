...
| 5 — Merge + archive | в работе | 2026-09-07 | PR #2314 integration → main открыт (`00796347`; origin/main `fcd36c5f` влит в ветку, конфликты только в снимке сети и конфиге декомпозиции); мердж — слово владельца; `RETROSPECTIVE.md` написана; карточка — `task:archive` после мерджа. Поверх: #2310 — PR #2315 (`ef18bb5d`, base = integration, сложен над #2314; после его влития base → main), дерево `Membrana-cw-window`; сверено координатором: client 15/122, device-board 1/5, контракты A–C не тронуты; живая проверка на приборе — за владельцем |
