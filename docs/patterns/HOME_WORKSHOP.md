...
| `usage` | что даёт вызов и как выглядит вывод — по глаголу (поправка 31.07) | 0..1, **необязательно** |

**Поправка 31.07 — [`AMENDMENT_F1`](../meeting/home-workshop/AMENDMENT_F1.md), ратифицирована
владельцем.** Поле `usage` — сосед `verbs`, не его замена: ключи `usage` ⊆ ключей `verbs`,
запись несёт `what` (что даёт вызов), `sample` (живой кусок вывода) и `measuredAt` (дата
прогона, `YYYY-MM-DD`).
