...
| `usage` | что даёт вызов, как выглядит вывод и какого рода evidence его подтверждает | 0..1, **необязательно** |
...
объявить `tool`. Пример несёт `what` (что даёт вызов), `sample` (кусок вывода),
`measuredAt` (дата замера, `YYYY-MM-DD`), `evidenceKind` (`run` — прожитый прогон;
`fixture` — воспроизводимая искусственная форма) и `source` (путь к носителю).
Fixture запрещено называть прожитым примером; отсутствие `run` остаётся видимым
пробелом даже при наличии fixture.
