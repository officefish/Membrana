...
- **HYGIENE ISSUE (must fix before any public deploy):** node `id`s embed the **absolute
  filesystem path incl. Windows username `user190825`** and the worktree name — appears
...
## Artifacts (committed to night branch)
- `artifacts/graph-core-scoped.html` — readable 477-node graph.html (owner visual eval)
- `artifacts/graph-core-scoped.json` — its graph.json
- `artifacts/GRAPH_REPORT-core-scoped.md` — scoped report
- `artifacts/GRAPH_REPORT-full-monorepo.md` — full-repo report (community structure)
- `artifacts/METRICS.md` — all numbers
- `artifacts/graphifyignore.used` — exact scope/ignore file used
- `/.graphifyignore` (repo root) — reproduction config
