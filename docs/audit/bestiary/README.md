...
### Что детектор считает носителем (B9 «Проза», #1204)

Утверждение честно, когда оно **опровержимо машиной**. Носителем участника считается имя,
встречающееся в машинном доме, а не в абзаце:

| Дом носителя | Что доказывает |
|---|---|
| `scripts/ask-persona.mjs`, `scripts/consilium.mjs` | участника можно вызвать |
| `scripts/lib/llm-procedures.json` | у процедуры есть провод к каналу |

`leadPersona` в описи носителем **не** считается: это строка-держатель, а не вызов — ровно
эту подмену вскрыл прецедент 25.07. Имена в домах носителя латиницей (`ozhegov`), в прозе —
русские и склоняемые, поэтому сверка идёт через транслит и корень (`scripts/lens-run.mjs`).

`README.md` и `SKILL.md` домами носителя **не являются** — абзац в них и есть предмет ловли.
Образец работающего контракта в дереве — опись кита: «в наборе эти файлы этих версий»
опровергается сверкой `yarn kits:audit` за секунды, поэтому не врёт. Проза в процедурах
допустима (норма владельца 25.07), но проверку не проходит.

### Легальное «нет» (B10 «Заглушка», #1219)

**Прежде чем делать поле обязательным — назови его легальное «нет».** Заглушку рождает не
лень, а принуждение: поле обязательно, честно заполнить нечем, зуб не пускает. Явное «нет»
(`declared-not-built`, `null`, статус с причиной рядом) заглушкой не считается и детектором
не ловится — честное отсутствие не дефект.
