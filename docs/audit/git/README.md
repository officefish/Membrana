...
| `registry/branches-by-category-YYYY-MM-DD.md` | Реестр 7 категорий (dated snapshot) | да |
| `registry/branches-by-category.md` | Текущий реестр внутри контейнера (overwrite content) | да |
...
3. Сценарии:
   - **A:** «Собери реестр веток по категориям» → пишет dated `registry/branches-by-category-YYYY-MM-DD.md` и перезаписывает `registry/branches-by-category.md`.
   - **B:** «Глубокий разбор категории N» → читает `registry/branches-by-category.md`, пишет `analysis/…`.
