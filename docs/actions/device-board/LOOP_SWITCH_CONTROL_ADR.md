...
### Р3 — НЕ вводить явные palette-узлы перехода (реализован вариант B, 2026-07-12)
- **Решение владельца 2026-07-12:** вариант **B** — рантайм-контракт + **read-only индикатор активного лупа на полотне** (реализован: `active-loop-indicator.ts` + `BoardRuntimeStatus`, показывает луп + причину manual/detection). Явные узлы (вариант A) → инсайт `insight-explicit-loop-switch-nodes` (deferred), вес пересматриваем по практике B.
