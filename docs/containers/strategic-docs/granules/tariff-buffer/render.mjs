/**
 * Function granule: tariff-buffer@1.0.0
 * Одна строка матрицы «Буфер записи × три тарифа». Значения — ТОЛЬКО из resource.json рядом;
...
export async function renderTariffBufferRow(_input, _io) {
  const raw = JSON.parse(readFileSync(new URL('./resource.json', import.meta.url), 'utf8'));
  const parsed = parseResource(raw);
  if (!parsed.ok) {
    throw new Error(`tariff-buffer/resource.json: ${parsed.findings.map((f) => `${f.where}: ${f.reason}`).join('; ')}`);
  }
