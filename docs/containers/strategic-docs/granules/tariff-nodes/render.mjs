/**
 * Function granule: tariff-nodes@1.0.0
 * Одна строка матрицы «Приборы × три тарифа». Значения — ТОЛЬКО из resource.json рядом;
...
export async function renderTariffNodesRow(_input, _io) {
  const raw = JSON.parse(readFileSync(new URL('./resource.json', import.meta.url), 'utf8'));
  const parsed = parseResource(raw);
  if (!parsed.ok) {
    throw new Error(`tariff-nodes/resource.json: ${parsed.findings.map((f) => `${f.where}: ${f.reason}`).join('; ')}`);
  }
