/**
 * Function granule: tariff-instruments@1.0.0
 * Несколько строк матрицы «инструмент × три тарифа» — по одной на запись entries[] resource.json.
 * Значения — ТОЛЬКО из resource.json рядом; слова и порядок колонок — из ядра паспорта.
 * Шапку таблицы даёт шаблон tariff-matrix.
 *
...
export async function renderTariffInstrumentsRows(_input, _io) {
  const raw = JSON.parse(readFileSync(new URL('./resource.json', import.meta.url), 'utf8'));
  const parsed = parseResource(raw);
  if (!parsed.ok) {
    throw new Error(`tariff-instruments/resource.json: ${parsed.findings.map((f) => `${f.where}: ${f.reason}`).join('; ')}`);
  }
