/**
 * Function granule: tariff-cold@1.0.0
 * Одна строка матрицы «Холодное хранилище × три тарифа». Значения — ТОЛЬКО из resource.json рядом;
...
export async function renderTariffColdRow(_input, _io) {
  const raw = JSON.parse(readFileSync(new URL('./resource.json', import.meta.url), 'utf8'));
  const parsed = parseResource(raw);
  if (!parsed.ok) {
    throw new Error(`tariff-cold/resource.json: ${parsed.findings.map((f) => `${f.where}: ${f.reason}`).join('; ')}`);
  }
