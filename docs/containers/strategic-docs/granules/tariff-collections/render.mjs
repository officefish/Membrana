/**
 * Function granule: tariff-collections@1.0.0
 * Одна строка матрицы «Коллекции (горячая память) × три тарифа». Значения — ТОЛЬКО из resource.json рядом;
...
export async function renderTariffCollectionsRow(_input, _io) {
  const raw = JSON.parse(readFileSync(new URL('./resource.json', import.meta.url), 'utf8'));
  const parsed = parseResource(raw);
  if (!parsed.ok) {
    throw new Error(`tariff-collections/resource.json: ${parsed.findings.map((f) => `${f.where}: ${f.reason}`).join('; ')}`);
  }
