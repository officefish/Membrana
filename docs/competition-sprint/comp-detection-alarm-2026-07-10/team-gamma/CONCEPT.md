...
- 2α done @ commit: `de64f64e` — `default-usercase-detection-alarm-gamma.ts` (билдер, 6 веток,
  плакат ①–⑥, полная цепочка брифа) + community entry `usercase-detection-alarm-gamma`
  (каталог 8→9) + 9 тестов документа и обновлённые тесты каталога.
  Scoped CI device-board: **PASS** (lint + typecheck + test, 660 tests / 137 файлов).
  Уточнение ADR G5 по факту host-моста: клиентский `startAsyncJob` поддерживает только
  `track-upload` (`report-build` → reject) → combined-отчёт собирается ДЕТАЧЕД на
  `on-async-resolved` промиса загрузки трека (print④ → MakeCombinedReport → PublishReport →
  print⑤), а запись трека перенесена НА detected-ветку (тихие окна закрывают запись без
  загрузки). Композиция брифа сохранена, демо честное: ни одного rejected-job.
- 2β done @ commit: `c5e2c8fd` — runtime-smoke `src/runtime/detection-alarm-gamma-usercase.test.ts` (6 тестов,
  по образцу эпик-smoke #323, но на узлах реальных подграфов документа):
  detected-путь — fusion 0.8 → branch detected → combined-report (reporter через
  journal1→get-reporter, оба анализа + трек у хоста, ReportRef валиден) → publish в журнал;
  guard — молчащие детекторы → not-detected; alarm — approaching → valid → true-ветка в ∞,
  lost → invalid → false-ветка = print «дистанция потеряна», выхода назад в ∞ нет;
  идемпотентность повторного exec combined-report (host-хэш входов); print③ печатает
  fusion value. Карточка в пикере: community ⇒ виден клиенту без entitlement.
  Полный scoped CI `turbo run lint typecheck test --filter=@membrana/device-board
  --filter=@membrana/client`: **PASS** — device-board 666 tests / 138 файлов
  (baseline 650 не сломан, +15 наших), client 287 tests / 77 файлов.
