...
## Статус установки (2026-06-27, текущая сессия)

**Порядок:** сначала M2 (headroom), потом M1 (codebase-memory-mcp).

### Окружение (всё уже установлено, но не в PATH по умолчанию)

| Компонент | Путь | Как добавить в PATH |
|-----------|------|---------------------|
| Rust / cargo | `%USERPROFILE%\.cargo\bin` | `$env:PATH += ";$HOME\.cargo\bin"` |
| Python 3.14.3 | `%USERPROFILE%\.local\bin\python3.14.exe` (через uv) | использовать полный путь |
| MSVC link.exe | `C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC\14.44.35207\bin\Hostx64\x64` | `$env:PATH += ";C:\Program Files (x86)\..."` |
| vswhere.exe | `C:\Program Files (x86)\Microsoft Visual Studio\Installer` | в PATH не нужен, достаточно link.exe |

### Проблема M2: Windows SDK не установлен

`headroom-ai[mcp]` компилируется через maturin (Rust+PyO3). При сборке падает:  
`LNK1181: не удается открыть входной файл "kernel32.lib"`

**Причина:** Windows 10 SDK установлен частично — в `C:\Program Files (x86)\Windows Kits\10\` есть только `Catalogs` и `Redist`, нет `Lib\` и `Include\`.

**Решение:** запустить VS Installer → Build Tools 2022 → Изменить → Отдельные компоненты → добавить **Windows 10 SDK (10.0.x)** → Установить.

### venv для headroom

Создан: `tools/headroom-venv` (Python 3.14.3, через `uv venv`)  
Команда установки после исправления SDK:
```
uv pip install --python C:\Users\user190825\practice\Membrana\tools\headroom-venv\Scripts\python.exe "headroom-ai[mcp]"
```

### M1: codebase-memory-mcp

Бинарь уже есть: `tools/bin/codebase-memory-mcp.exe` (269 МБ).  
Установка в систему (`install.ps1`) — после завершения M2.
