...
>
> **ПОПРАВКА СКОУПА после ревью (тот же день):** обнаружена регистрация
> `panel-detector-compare-board` #452 соседней сессией (консилиум
> detector-compare-board-2026-07-14, видение владельца, «office не трогаем»).
> QC2 (office-модуль benchmark, benchmark-push, DetectorCompareBoard) удалён
> из ветки ДО мёржа — LGTM ниже остаётся валиден для оставшегося подмножества
> (drift-борд + shell-навигация + lib): удаление кода не добавляет рисков,
> прогоны перепройдены на финальном SHA. Tier после сужения: T1 (panel + scripts
> удалены, office не изменён).

Tier: T2 → T1 (после сужения скоупа)
