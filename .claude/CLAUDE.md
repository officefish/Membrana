...
5. **Только теперь** — извлечение памяти. С 28.07 оно едет НА ласточке само: `telegram:swallow` после `sent=true` запускает `persona-memory:extract --all` (помеха №2 счётчика чистых прогонов); руками `yarn persona-memory:extract --all` — только при сбое авто-шага.
   Требование 3 из #569: в память попадает лишь то, что уже показано партнёрам.
   Поэтому извлечение памяти **вынуто из цепочки** `ritual:evening` (18.07) — цепочка
   обрывается на черновике и ждать одобрения не умеет. Побочно чинится второй порядковый
   дефект: раньше извлечение шло 4-м шагом, до того как `team-evening-feedback` породит
   сегодняшний протокол, и сегодняшний фидбек попадал в память лишь на следующие сутки.
...
**Снос деревьев — только через `yarn repo:clean --execute --worktrees`.** Raw
`git worktree remove` — тем более циклом — запрещён. Снос дерева это контролируемый снос
(ADR-0020): связи наружу снимаются КАК связи до удаления, пост-чек живых деревьев идёт
после КАЖДОГО сноса. Raw-команда не делает ни того, ни другого, и рекурсивное удаление
уходит сквозь junction `node_modules/@membrana/*` в `packages/*` соседа. Цена класса за
17 дней: 2019 → 2097+2089 → **2152 файла** (06.08, единственный раз мимо инструмента —
[прецедент](../docs/precedents/2026-08-06-worktree-remove-bypassed-controlled-demolition.md)).
Отдельно: дерево ВНУТРИ другого дерева (`.claude/worktrees/…`, `.worktrees/…`) вложенность
своей не объявляет — проверять до сноса.
