...
/**
 * Решение по ВЛОЖЕННОМУ ресурсу — набору или пробе внутри прибора.
 *
 * ПОЧЕМУ ОТДЕЛЬНОЙ ФУНКЦИЕЙ, А НЕ ПРОВЕРКОЙ В МЕСТЕ ЗАПРОСА. Ревью #2276 нашло настоящую дыру:
 * список проб шёл сразу в `count`/`findMany` по `{deviceId, collectionId}`, и НЕСУЩЕСТВУЮЩИЙ
 * набор давал `200` с пустым списком. Документ ручки при этом обещал `404`. Два объявления
 * одного факта разошлись — и разошлись в УСПОКАИВАЮЩУЮ сторону: партнёр с опечаткой в
 * идентификаторе читал «набор пуст» вместо «набора нет» и не знал, что ошибся.
 *
 * ПОРЯДОК ЗДЕСЬ ТОЖЕ НЕСУЩИЙ: вердикт по прибору идёт ПЕРВЫМ. Для чужого прибора мы обязаны
 * ответить `403`, НЕ раскрывая, есть ли внутри такой набор, — иначе `404`/`403` начнут
 * рассказывать о содержимом чужого прибора. Проверка существования набора выполняется только
 * после того, как доступ к прибору разрешён.
 */
export function accessForNested(deviceAccess: OpenApiAccess, nestedExists: boolean): OpenApiAccess {
  if (deviceAccess !== 'allow') return deviceAccess;
  return nestedExists ? 'allow' : 'absent';
}
