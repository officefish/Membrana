...
/**
 * ЗАГРУЗКА ЯДРА — ВНЕ ИЗМЕРЯЕМОГО ВРЕМЕНИ ТЕСТА.
 *
 * Сервис тянет ядро отбора динамическим импортом (пакет ESM, этот — CommonJS), и цену первой
 * загрузки платил ПЕРВЫЙ тест файла. На тёплом дереве это 2 с из 5 допустимых, на холодном —
 * больше порога, и файл краснел таймаутом. Таймаут — симптом, а не вердикт: он говорит
 * «долго», а не «предмет проверки сломан», и от снятия самого предмета такой тест покраснел
 * бы ровно так же. Прогрев переносит цену в подготовку, и дальше таймаут судит только предмет.
 *
 * Это НЕ маскировка: медленная загрузка остаётся фактом и видна на старте сервиса; здесь
 * лишь сказано, что она не является предметом ЭТИХ зубов.
 */
beforeAll(async () => {
  await import('@membrana/media-library-service');
});
