/**
 * WAV PCM16 → моно Float32. Третья копия одного и того же (media `decode-wav-mono.ts`,
 * scripts `wav-read.mjs`) — намеренно: пакет framework-нейтрален и в media/scripts не тянется.
 * Кандидат на вынос в `packages/libs` — названо в отчёте PR-3, здесь не решается.
 */
