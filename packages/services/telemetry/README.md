...
### Теги отчётов анализа

Поле `tags` на записи (не внутри `data`). Для FFT threshold-test: `analysis` (всегда), `detection` или `clear`, плюс теги источника (`fft`, `threshold-test`, …). `getStats()` считает `detection` / `clear` по `entry.tags` (устаревшие `detected` / `not-detected` / `drone` / `calm` учитываются для совместимости).
