...
export interface BufferStopVerdict {
  ...
  /** Возобновится ли запись сама, когда место освободится (вопрос 2). */
...
export function stopDecision(
  ...
  if (!(limitBytes > 0)) {
    ...
    return {
      ...
      say: `${what} идёт: предел буфера не объявлен, судить о заполнении нечем`,
    };
  }
...
function sayOf(
  ...
  if (action === 'stop') {
    ...
    return `${what} остановлена: буфер заполнен на ${percent}%, ${left}. Запись продолжится сама, когда место освободится — уберите лишнее в «Управлении буфером».`;
  }
  if (action === 'warn') {
    return `${what} идёт, но буфер заполнен на ${percent}%: ${left}. Пора убрать лишнее.`;
  }
  return `${what} идёт: буфер заполнен на ${percent}%, ${left}`;
}
