...
describe('три постулата названы полями, а не спрятаны в коде', () => {
  it('вопрос 2 — остановка ДО ОСВОБОЖДЕНИЯ: вердикт говорит это полем и словом', () => {
    ...
    // Ядро обещает ВОЗМОЖНОСТЬ, а не чужое поведение: возобновляет тот, кто останавливал
    // (ревью #2214 поймало обещание без исполнителя).
    expect(v.say).toMatch(/станет возможна снова, когда место освободится/u);
