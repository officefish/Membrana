  ...
  private async pullRemote(): Promise<void> {
    ...
    try {
      if (this.port.listJournalItems) {
        ...
        const localCount = (await this.local.listItems()).length;
        if (
          this.lastRemoteJournalTimestamp !== null &&
          pulled.counts?.all !== undefined &&
          pulled.counts.all < localCount
        ) {
          const full = await this.fetchRemoteItemsFromUnifiedApi(null);
          this.local.reconcileRemoteItems(full.items, this.pendingPushEntryIds);
          this.replaceRemoteWatermark(full.items);
        } else if (this.lastRemoteJournalTimestamp === null) {
