...
describe('parseOfficeEnv — office pair for cabinet registration', () => {
  ...
  it('throws for an invalid OFFICE_URL when the full pair is configured', () => {
    expect(() => parseOfficeEnv({ OFFICE_URL: 'not a url', OFFICE_API_TOKEN: 'shared-office-token' })).toThrow(
      /OFFICE_URL must be a valid URL/,
    );
  });
