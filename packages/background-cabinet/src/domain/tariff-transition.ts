 ...
 *
 * `self` (#2281, слово владельца 04.09) — СОБСТВЕННЫЙ ВЫБОР владельца мембраны. Третье
 * основание заведено отдельным именем, а не подведено под `admin`: журнал смены обязан
 * пережить саму смену и ответить «на каком основании» (вердикт M6), а «владелец сам выбрал» и
 * «администратор перевёл» — разные события. Слив их, через месяц нельзя было бы отличить
 * решение человека от действия поддержки.
 *
 * ВОРОТА ЗА `self` СЕГОДНЯ ОТКРЫТЫ. Оплата и промокод как УСЛОВИЕ перехода — следующий билет;
 * место, где они закроются, названо одним предикатом (`selfTransitionGate`), чтобы закрыть их
 * позже, не переписывая переход.
 */
export type TariffChangeProof = 'admin' | 'promo' | 'self';
 ...
 *
 * Список — МАССИВ, а тип выведен из него (#2281). Раньше тип был написан руками, а копия списка
 * жила в тесте под обещанием «новая причина без зуба = красный». Обещание не исполнялось: тесты
 * исключены из `tsc` (`exclude` тест-файлов), а vitest типы не проверяет — сверять там было
 * нечему и нечем. Теперь сверять НЕЧЕГО: список один, и второй копии, способной разъехаться,
 * просто нет.
 *
 * `promo_not_single_use`: код выпущен более чем на одно погашение — состояние, которого по норме
 * не существует (решение владельца, мостик 07.08): промокод одноразов, так и сказано в
 * комментарии модели. Виноват администратор, выпустивший код, а НЕ пользователь, поэтому подарок
 * не списывается. Причина живёт в закрытом списке, а не броском исключения и не проверкой в
 * сервисе: ответ пользователю обязан нести причину ИЗ ЭТОГО списка, и второй источник отказов
 * рядом с ним — та же болезнь, что «net» у снов (#1425).
 */
export const TRANSITION_DENY_REASONS = [
...
export type TransitionDenyReason = (typeof TRANSITION_DENY_REASONS)[number];
