...
| `yarn tooling:overview [--report]` | Stdout — живой агентский инвентарь; `--report` — тот же SCRIPTS_LIST (S2) |
...
Реестр контейнера: `scripts:registry --report` ≡ `tooling:overview --report`. Stdout overview **не** коммитить в AGENTS.
...
# эквивалент: yarn tooling:overview --report  (stdout overview остаётся; реестр — в stderr-метке)
# опционально: --dated · --cache-overview (дамп overview JSON в cache/, не SoT)
```
