...
test('спорные случаи разрешены каноном поимённо (Р4)', () => {
  ...
  assert.equal(areaOf('docs/design/REFERENCE_SOURCES.md'), 'product');
...
test('граница background-office / background-media (правка ADR-0013 при принятии)', () => {
  // office — шлюз интеграций Claude/Linear/GitHub, инфраструктура агентской работы.
  // Числился за кабинетом и приписывал ему работу, которой кабинет не касался.
  assert.equal(areaOf('packages/background-office/src/app.module.ts'), 'tooling');
  // media — пользовательские WAV, коллекции, шаблоны trends: кабинет верен.
  assert.equal(areaOf('packages/background-media/src/api.ts'), 'cabinet');
});
