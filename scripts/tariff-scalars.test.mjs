 ...
 *
 * #2333 v2 снимает с этой декларации авторство квот: живой автор — grid ->
 * cabinet projection. Тест оставлен как исторический зуб формы декларации и как
 * запрет вернуть seed к чтению scalars.
 *
...
test('носитель читает декларацию и не держит своих числовых констант', () => {
  ...
  assert.match(seed, /tariff-project-cabinet\.mjs/u, 'сид читает проекцию сетки');
  assert.doesNotMatch(seed, /tariff-scalars\.mjs/u, 'scalars больше не автор квот сида');
