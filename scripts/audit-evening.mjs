...
if (!head || head === base) {
  console.log(`За ${date} коммитов нет — отчёт будет нулевым (это факт, не сбой).`);
}
...
for (const line of git('diff', '--numstat', `${base}..${head}`).split('\n')) {
  ...
  // Переименование: git пишет `dir/{old => new}/file.ts` либо `old.ts => new.ts`.
  // Берём НОВЫЙ путь. Замена по каждой группе `{…}` отдельно и без жадности: прежний
  // `^.*\{.*=>\s*(.+)\}.*$` на пути с двумя группами хватал хвост (замечание ревью).
  const path = m[3].includes('{')
    ? m[3].replace(/\{[^{}]*?=>\s*([^{}]*?)\}/gu, '$1').replace(/\/{2,}/gu, '/')
    : m[3].replace(/^.*?\s=>\s(.+)$/u, '$1');
  fileStats.push({
    path,
