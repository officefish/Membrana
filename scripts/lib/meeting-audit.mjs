...
/** Единственное значение канала, при котором гейт посылок отработал ПРИ ЗАПИСИ. */
export const TOOL_CHANNEL = 'llm';

/**
 * Значение пометки канала протокола (`<!-- канал: llm … -->`), либо `null`.
 *
 * #1047: проверка [6] смотрела на НАЛИЧИЕ пометки и объявляла «произведён инструментом»
 * всё, что помечено. Три ручных протокола `tasks-workshop` с честной шапкой
 * `<!-- канал: hand — протокол вёл председатель вручную -->` получили зелёный «гейт посылок
 * отработал при записи», хотя `consilium.mjs` их не видел вовсе. Ложный ЗЕЛЁНЫЙ опаснее
 * ложного красного: регламент заседания стоит на том, что аудитор говорит правду о
 * процедуре, а не о наличии комментария.
 *
 * @param {string} md
 * @returns {string|null}
 */
export function protocolChannel(md) {
  return String(md ?? '').match(/<!--\s*канал:\s*([\wа-яё-]+)/iu)?.[1]?.toLowerCase() ?? null;
}
...
export function auditMeeting(state) {
  ...
  for (const p of state.protocols) {
    ...
    const channel = protocolChannel(p.md);
    checks.push({
      n: 6,
      subject: p.file,
      status: channel === null ? 'FAIL' : channel === TOOL_CHANNEL ? 'PASS' : 'ПРЕДУПР',
      note:
        channel === null
          ...
          : channel === TOOL_CHANNEL
            ...
            : `канал «${channel}»: протокол вёлся вручную, гейт посылок при записи НЕ вызывался — ` +
              'секции проверены только постфактум (проверки 1–4 выше)',
    });
  }
