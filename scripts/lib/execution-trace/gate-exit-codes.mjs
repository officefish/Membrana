...
export const FINDINGS = Object.freeze({
  ...
  /**
   * Самый ранний след блока раньше `window.from` — окно назначено догадкой ПОЗЖЕ начала
   * работ (шот A, 03.08). Основание: два рецидива за два дня — оба лечились перерезкой v3
   * окна с ратификацией владельца, при том что следы были честны и врало окно. Одна находка
   * на БЛОК (объект лечения — окно, не следы); порог строго `<` без допусков: календарных
   * чисел владелец не называл (прецедент isStale). Лечится перерезкой ОКНА — и потому
   * считается от текущего ратифицированного `from`: перерезанное окно вылечено по построению.
   */
  WINDOW_AFTER_FIRST_TRACE: 'eg-window-after-first-trace',
