 ...
 * решением о её роде.
 *
 * 29.08: ритм 14 → 15. Заведена `ritual-night` — дверь ночи наравне с утром и вечером.
 * Род `ритм` решением держателя: ночь идёт по расписанию суток и живёт в одной шкатулке с
 * `ritual-day` и `ritual-evening` — те же три полосы, тот же глагол-дверь, та же лента прогонов.
 * Route-контракта рода «разработка» у неё нет: полосы не имеют отдельных держателей-этапов.
 * Названная цена: если у ночи вырастут этапы с разными держателями и переходами между ними,
 * род придётся переводить в `разработка` — ровно та же оговорка, что записана для деплоя.
 *
