...
/**
 * Решение гейта.
 * `artifact` — необязательный признак файла ревью от вызывающего: `{exists, path, searched?}`.
 * `searched` — просмотренные пути (все рабочие деревья); попадает в причину «не найдено».
...
export function reviewGateDecision({ headSha, currentBase, verdict, override, scope, artifact } = {}) {
  ...
  if (!verdict) {
    ...
    // ГДЕ ИСКАЛИ — часть диагноза (вещдок 22.08): артефакт не коммитится (.gitignore:154),
    // лежит в том дереве, где гоняли code-review, а сессии работают в разных деревьях.
    // Молчание о просмотренных путях превращало ненайденный файл в загадку и толкало
    // переносить его руками вместо того, чтобы сказать, куда смотрели.
    const where = artifact?.searched?.length ? ` · искал: ${artifact.searched.join(' · ')}` : '';
    return { state: 'unknown', reason: `ревью тимлида по этому PR не найдено — прогнать: yarn code-review:pr <N> (вердикт привяжется к ${headSha.slice(0, 8)})${where}` };
  }
