...
test('Promise вместо состояний — отказ, а не таблица «не запрошен»', () => {
  // Необёрнутое обещание прошло бы дальше молча и наполнило таблицу пустыми строками —
  // то есть соврало бы тем же способом, от которого модуль и заведён.
  assert.throws(
...
test('синхронный источник проходит — контракт не сломан лишней строгостью', () => {
  const { block } = buildReferencedStatesBlock('правим #1562', () => ({
    unknown: false,
    states: { 1562: 'MERGED' },
    missing: [],
  }));
