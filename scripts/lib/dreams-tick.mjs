...
/**
 * Роды, при которых кубик НЕ крутится, — названы поимённо и с причиной.
 *
 * `ok` — работа сделана, крутить нечего.
 *
 * `panel_unreachable` — РЕШЕНИЕ ВСЛУХ 11.09 (спринт network-panel-refusal, #1449).
 * Род внесён в перечень контейнера и обязан был получить ответ: крутит ли он кубик.
 * Ответ — НЕТ, по двум причинам. Первая: `classifyOutcome` его не выдаёт вовсе
 * (см. classify.mjs — панель, ответившая чем угодно, жива и родом не является), он
 * рождается ДО первой попытки к звеньям, в зонде достижимости панели. Второй провайдер
 * тут не помощник: отказала не сторона провайдера, а предпосылка всей цепочки.
 * Вторая: если бы он всё же дошёл сюда как failover, кубик перебрал бы ВСЕХ провайдеров
 * об одну и ту же мёртвую панель — ровно то «четыре раза неизвестно», ради которого
 * спринт и затевался. Failover лечит «этот провайдер упал, другой может ответить»;
 * «до провайдеров не дошло» он не лечит, а маскирует.
 */
export const NON_FAILOVER_OUTCOMES = new Set(['ok', 'panel_unreachable']);

/**
 * Роды, при которых кубик крутит следующего провайдера.
 *
 * Перечислены поимённо, а не выведены как «всё кроме ok»: когда в перечне
 * контейнера появится новый род, зуб `FAILOVER_OUTCOMES = OUTCOME_IDS \ NON_FAILOVER`
 * покраснеет и потребует решения вслух — род, не названный НИ В ОДНОМ из двух
 * множеств, красит зуб. Молчаливое «новое — тоже failover» повторило бы ровно ту
 * ошибку, которую этот шов чинит.
 */
