...
export function buildPersonaTraceLayer(repoRoot, date, deps = {}) {
  ...
  //
  // #2418: `gated` НЕ идёт в `problems`. Проблема — то, что могло бы быть иначе; здесь
  // иначе быть не может. `day-memo` стоит в вечерней цепочке РАНЬШЕ, чем
  // `team-evening-feedback` рождает протокол сегодняшнего показа, поэтому на момент
  // шага протокола нет ВСЕГДА — по построению порядка, а не по стечению обстоятельств.
  // Через `problems` это становилось exit 3, то есть находкой, и вечер за вечером
  // (17, 18, 21, 22.09 — по четыре трения в день, амандментов ноль) журнал получал
  // непогашенное трение о том, что цепочка идёт в своём порядке. Находка, которая
  // срабатывает всегда, не отличает день от дня и потому не находка.
  //
  // Состояние никуда не делось: оно в `stats.gated`, в разделе слоя и отдельной строкой
  // хендоффа у сборщика (`day-memo.mjs`). Убрана ровно одна из двух деклараций — та,
  // что превращала норму в находку.
