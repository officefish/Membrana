...
/**
 * Заголовок секции ХВОСТОВОГО эха — полного дубля вопроса и повестки в конце протокола.
 *
 * Правило 7 регламента велит класть полную копию вопроса и повестки в хвост сборки.
 * Полгода `protocolBody` резал только голову — и потому весь этот дубль попадал в зону
 * сканирования. Следствие: как только формат НАЧИНАЕТ соблюдать правило 7, `rt-6`
 * находит любую метку повестки в её же копии и говорит «покрыто», не спросив, произнесла
 * ли её хоть одна роль. Проверка вырождается в тавтологию — тем вернее, чем дисциплинированнее
 * протокол. Аудит `workshop-wires` подтвердил слепоту четырежды (M6, M9, M7, M5); на M9 тем же
 * путём прошёл ложный зелёный гейта посылок: строка «**Список посылок обязателен.**» из
 * ХВОСТОВОЙ копии повестки была принята за заголовок настоящей секции, а телом секции стала
 * цитата предмета. Реальная секция комнаты называлась «Посылки (обязательный список)» и не
 * матчилась вовсе — то есть гейт с правом отказа защитил зря.
 *
 * Форма заголовка в живых протоколах плавает («Полное эхо вопроса», «Полное эхо: повестка»,
 * «Повестка (полное эхо)»), поэтому ловим не шаблон, а два устойчивых сочетания. Слова
 * «эхо» ОДНОГО мало: заголовки вида «Тема 1 — эхо-камера» и «Повестка: … и эхо-камера»
 * живут в теле обычных консилиумов, и срез по ним снёс бы вердикт вместе с посылками —
 * ложный красный, который эта библиотека считает дороже пропущенного дефекта (#1046).
 *
 * Считаем только `#`-заголовок: `**жирная**` строка носителем хвостового эха ни в одном из
 * двенадцати протоколов не была, а поблажка к ней вернула бы ровно тот риск, ради которого
 * сочетания и сужены.
 */
const TAIL_ECHO_HEADING_RE = /^\s{0,3}#{1,4}\s.*(полное\s+эхо|эхо\s*:?\s*(вопрос|повестк))/iu;

/**
 * Тело протокола — реплики и итог, БЕЗ эха повестки: ни головного, ни хвостового.
 *
 ...
 * ID назван в РЕПЛИКЕ или ИТОГЕ. Отсекаем всё до первой реплики `[Роль]:` — и всё от
 * хвостовой копии повестки до конца файла.
 *
 * Хвост режется ПО ЗАГОЛОВКУ ЭХА, а не «после последней реплики»: между последней
 * репликой и хвостом лежат секции, на которых стоят три другие проверки — итоговое
 * решение (`hasVerdictSection`), список посылок (`premisesSection`), контракт и запреты.
 * Срез по последней реплике обнулил бы их все разом.
 *
 * Эхо идёт до конца файла: оно содержит повестку целиком, вместе с её собственными
 * заголовками, — значит «до следующего заголовка» тут не работает.
 */
export function protocolBody(protocolMd) {
  ...
  if (firstReply === -1) return '';
  const afterHead = lines.slice(firstReply);
  const tailEcho = afterHead.findIndex((l) => TAIL_ECHO_HEADING_RE.test(l));
  return (tailEcho === -1 ? afterHead : afterHead.slice(0, tailEcho)).join('\n');
}
