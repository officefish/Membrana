...
/**
 * Нарушение повестки ЗАСЕДАНИЯ: она обязана нести ровно ОДИН ID-вопрос (S-Z1).
 *
 * Регламент: `docs/ZASEDANIE_REGULATION.md`. Почему отказ, а не предупреждение —
 ...
 * нечего: вердикт либо есть, либо заседания не было (S-Z2).
 *
 * З0 (установочное) — не исключение: его единственный вопрос «какой порядок
 ...
 */
export function zasedanieAgendaProblem(topicMd) {
  const ids = extractAgendaIds(topicMd);
