...
export function projectMarkdown(a) {
  ...
  lines.push('');
  // Метки классов — русские: формат живых журналов неизменен для ВСЕХ читателей
  // (parseJournalMd, team-memory-report, day-memo trace) — латиница дала ложный
  // масс-дифф на первом же бою 28.07.
  const CLASS_RU = { position: 'позиция', insight: 'инсайт', precedent: 'прецедент', routine: 'рутина' };
  for (const r of a.retained) {
    ...
    lines.push(`### ${date} · ${CLASS_RU[r.class] ?? r.class ?? 'рутина'} · ${slug}${pin}`);
    lines.push('');
