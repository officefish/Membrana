...
/** Шаги цепочки из живого package.json. */
function chainSteps(name) {
  const chain = JSON.parse(readFileSync(join(repoRoot, 'package.json'), 'utf8')).scripts[name];
  return chain.split('&&').map((s) => s.trim());
}

/**
 * ВСЕ цепочки, генерирующие план. Список выведен из package.json, а не захардкожен:
 * ревью 17.07 показало, что C1 был починен наполовину — `ritual:day:no-api`
 * (fallback при пустом кредите) генерировал план ВООБЩЕ без гейта, и правка первой
 * цепочки этого не касалась. Тест, знающий только про `ritual:day`, охранял бы одну
 * дверь из двух.
 */
function chainsThatGeneratePlan() {
  const scripts = JSON.parse(readFileSync(join(repoRoot, 'package.json'), 'utf8')).scripts;
  return Object.keys(scripts).filter((k) => scripts[k].includes('main-day-issue.mjs') && scripts[k].includes('&&'));
}

test('гейт посылок стоит ПЕРЕД генератором плана — во ВСЕХ цепочках (C1 #572)', () => {
  const chains = chainsThatGeneratePlan();
  assert.ok(chains.length > 0, 'хоть одна цепочка генерации обязана существовать');

  for (const name of chains) {
    const steps = chainSteps(name);
    ...
    assert.ok(probeAt >= 0, `${name}: генерирует план без гейта main-day-probe — посылки не проверяются вовсе`);
    assert.ok(
      probeAt < generatorAt,
      `${name}: гейт обязан стоять ДО генератора, иначе его «генерация заблокирована» — ложь ` +
        `(пробник ${probeAt + 1}-й, генератор ${generatorAt + 1}-й)`,
    );
  }
});

test('гейт не обёрнут `|| true` — иначе нарушенная посылка не остановит день', () => {
  for (const name of chainsThatGeneratePlan()) {
    const probeStep = chainSteps(name).find((s) => s.includes('main-day-probe.mjs'));
    assert.doesNotMatch(
      probeStep ?? '',
      /\|\|\s*true/u,
      `${name}: блокирующий гейт под \`|| true\` — формальность, exit 2 будет проглочен (урок B1 #572)`,
    );
  }
});
