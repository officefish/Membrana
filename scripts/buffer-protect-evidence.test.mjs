...
test('НЕПОЛНОЕ ЧТЕНИЕ — отказ: недочитанный буфер занижает число незащищённых', () => {
  const script = readFileSync(SCRIPT, 'utf8');
  assert.match(script, /PAGE_SIZE = 100/u, 'страница по потолку сервера, а не по желанию');
  assert.match(script, /буфер недочитан, судить по части нельзя/u, 'неполнота — отказ, а не тишина');
});
