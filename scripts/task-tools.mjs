#!/usr/bin/env node
/**
 * yarn task:tools — таблица инструментов мастерской docs/tasks.
 *
 *   yarn task:tools
 *   yarn task:tools --json
 *   yarn task:tools --zone workshop|contract|neighbor
 *   yarn task:tools --doc <id>
 *
 * Канон: docs/tasks/WORKSHOP.md · kits/tasks-master · скилл membrana-tasks-workshop.
 */

import { dirname, resolve } from 'node:path';
import { fileURLToPath } from 'node:url';

import { inventoryWorkshopTools, readToolDoc } from './lib/task-tools.mjs';
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--json') json = true;
      ...
      console.log(`Usage: yarn task:tools [--json] [--zone workshop|contract|neighbor] [--doc <id>]`);
      process.exitCode = 0;
      return null;
    }
  }
...
if (!args) {
  ...
  if (!res.ok) {
    console.error(`task:tools --doc: ${res.error}`);
    process.exitCode = 1;
  } else if (args.json) {
