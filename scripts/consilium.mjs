...
export function buildPrompt({ question, topicFile, ghIssueData, noContext, orderedRoles, minReplies, ragBlock, withMemory = false, meeting = '' }) {
  ...
  // Тот же зверь, вторая голова, пойман 30.07 на M0 workshop-wires: вопрос-то доехал, а
  // ПОВЕСТКА (3630 символов с девятью кандидатами) стояла только в хвосте и была срезана
  // потолком целиком. Комната честно отказалась ранжировать то, чего не видела, и назвала
  // анти-результат «скопировать ряд из соседнего заседания». Манифест входа показал
  // «повестка — не доехал» — спор стал фактом (#1487).
  //
  // Лечение то же: повестка идёт В ГОЛОВУ, сразу за вопросом. Хвостовое эхо сохранено —
  // конец сборки остаётся сильнейшей позицией для формата, а голова гарантирует доставку.
  if (topicFile) {
    const headAgenda = read('повестка (эхо в голове)', topicFile, MAX_TOPIC_CHARS);
    parts.push(`## Повестка (${topicFile}) — полное эхо в конце сборки`, '');
    parts.push(place(headAgenda), '', '---', '');
  }
...
export function wrapSeanseFile({ body, question, orderedRoles, model, ghIssue, topicFile, relPath, inputManifest }) {
  ...
  const meta = [
    ...
    // инструментом, от записанного в seanses напрямую (мимо гейта).
    //
    // #696: пометка вшивается ЗДЕСЬ, в wrapSeanseFile — ДО прогона гейта структуры
    // вердикта ниже. Прежний текст «гейт структуры вердикта пройден» был кругово́е
    // самозаверение: в момент штампа исход гейта ещё неизвестен (а на не-meeting
    // прогоне гейт вообще не запускается). Хуже — при провале fileBody целиком
    // уходит в docs/seanses/rejected/, унося ложное «пройден» в отклонённый черновик.
    // Итог гейта выражает МЕСТО файла (seanses = принят, rejected/ = отклонён), а не
    // текст пометки; поэтому пометка несёт лишь канал, без заявления об исходе.
