...
// После разбора (4 сняты, 5 в pending) тот же зуб обязан быть зелёным — иначе разбор
...
test('перечень pending покрывает ровно пять проводов и все с причиной и датой', () => {
  const report = runCheck({ today: TODAY });
  assert.equal(report.findings.length, 0);
  // Сдвинем «сегодня» за срок — все пять обязаны проявиться как просроченные.
  const after = runCheck({ today: '2026-08-10' });
  assert.equal(after.findings.length, 5, 'за сроком должны проявиться все пять');
