 ...
 *
 * Лицензия контрактов процедурного слоя (Ф3/Ф4 #1220).
 * --check  — зуб: все контракты valid (exit 1 иначе; legacy = fail)
 * --write  — перегенерировать проекции со штампом parser@ (миграция legacy)
 * (без флагов — отчёт)
 */
