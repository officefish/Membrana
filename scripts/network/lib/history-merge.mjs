 ...
 *
 * ГРАНИЧНЫЙ СЛУЧАЙ, названный вслух (замечание ревью PR #2340): две ветки пишут замер с
 * одинаковым `at` до миллисекунды И с одной машины. Ключ их схлопывает — и это решение,
 * а не побочный эффект. Обоснование: момент `at` берётся с часов машины в начале прогона
 * зондов, а прогон занимает секунды; два РАЗНЫХ замера одной машины не могут разделять
 * миллисекунду, потому что второй начался бы после первого. Совпадение до миллисекунды
 * означает либо один замер, записанный дважды (обычный случай — обе ветки несут одну
 * строку от общего предка), либо два прогона, запущенных из одного момента в разных
 * ветках, и тогда они меряют ОДНО состояние сети — хранить оба незачем.
 *
 * Что при этом НЕ замалчивается: если строки за один ключ различаются телом, `mergeHistories`
 * называет это в `redefined` и берёт нашу сторону детерминированно. То есть схлопывание
 * видно в выводе драйвера, а не происходит молча.
 *
 * Чего ключ НЕ покрывает: две РАЗНЫЕ машины в один момент — это два независимых замера, и
 * host в ключе держит их раздельно (зуб «один момент, РАЗНЫЕ машины»).
 */
