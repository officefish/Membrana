 ...
 *  - Студия (интерфейс и доска) работает от имени `device.mediaDeviceId` — его назвал кабинет
 *    в ответ на ключ доступа, вставленный человеком в панель связки;
 *  - служба узла — БЕЗГОЛОВЫЙ тракт по ADR-0027 — работает от имени `FIELD_NODE_DEVICE_ID` из
 *    своего `.env`: берёт задания «снять пробу N секунд» из очереди СВОЕГО прибора, снимает
 *    звук через ffmpeg, сдаёт наблюдение и шлёт пульс.
 *
 * 25.09 владелец перевязал прибор на другой кабинет: ключ доступа уехал в Студию, служба
 * осталась на прежнем приборе — и этот самый скрипт печатал «узлы к ночи готовы», потому что
 * судил только кабинетную половину. Расхождение двух привязок обязано быть отказом с названной
 * причиной, а не тишиной.
 *
 * ЧЕГО РАСХОЖДЕНИЕ НЕ ОБЪЯСНЯЕТ — и почему об этом написано здесь, в приборе. Первая
 * формулировка отказа говорила «пробы не поедут никуда», и это было лишним выводом: записи со
 * сценария доски отправляет САМА Студия
 * (`packages/services/media-library/src/backends/server-storage-backend.ts` — `POST
 * /v1/devices/<deviceId>/collections/<id>/samples` под её собственным `deviceId`), и после
 * перевязки она отправляет их от имени НОВОГО прибора, верно. Безголовый тракт узла к записи с
 * доски отношения не имеет. Поэтому отказ говорит ровно о том, что следует из расхождения:
 * задания и пульс безголового тракта уходят не туда. Причина отсутствия проб 25.09 остаётся
 * ненайденной, и приписывать её этой проверке нельзя — иначе прибор врёт в свою пользу.
 *
...
export function reconcileServiceBinding(device, nodeBinding) {
  ...
  return {
    checked: true,
    split: true,
    line: `РАСХОЖДЕНИЕ ПРИВЯЗОК: Студия на приборе ${studio}, служба узла на приборе ${service}${from} — задания и пульс безголового тракта узла уходят не туда`,
    remedy:
