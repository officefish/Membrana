...
// --- Тезис 3 (K→экстрактор): новый 5-блочный формат ---
test('extractDayDigestSlots: 5-блочный формат → payload с headline/highPriority/perspective', () => {
  const md = [
    '<!-- Сгенерировано: 2026-07-20T05:00:00.000Z (yarn main-day-issue) -->',
    '# Центральная задача дня — 2026-07-20',
    '## Магистраль',
    'Довести генератор стратегии до 5-блочного плана. Критерий — живой прогон.',
    '## Подкрепление',
    '- Барьер против проглоченного сбоя',
    '- Тест интеграции цепочки',
    '## Перспективные',
    '- Лендинг-соревнование',
    '## Экспериментальные',
    '— пусто —',
    '## Санитарные',
    '- Разобрать oversized-PR',
  ].join('\n');
  const p = extractDayDigest(md);
  assert.equal(p.kind, 'day');
  assert.equal(p.date, '2026-07-20');
  assert.match(p.headline, /Довести генератор стратегии/u);
  assert.equal(p.highPriority.length, 2);
  assert.deepEqual(p.perspective, ['Лендинг-соревнование']);
});

test('extractDayDigestSlots: без Магистрали → null (фолбэк на старый формат)', () => {
  assert.equal(extractDayDigestSlots('# Центральная задача дня — 2026-07-20\n## Прочее\nтекст'), null);
});
