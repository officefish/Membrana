...
const INPUT_DOCS = [
  ...
  {
    // У1 разбора #1539. Генератор НЕ был слеп: он читал assertions и имел прямой запрет
    // синтезировать своё. Беда в том, что выбор с гейта туда не доезжал — канон требует
    // перечеканки main-day-assertions.json при `magistral --choose`, и она не сделана.
    // Живой случай 31.07: владелец отчеканил mfcc-mic-plugins, гейт закрылся, а в
    // assertions осталась вчерашняя mfcc-spectral-core — документ раскрыл ВЧЕРАШНЮЮ
    // магистраль, и сессия правила его руками.
    // Правильная сторона починки — гейт (перечеканка), но morning-gate.mjs вне зоны блока.
    // Здесь генератор становится ОБОРОНИТЕЛЬНЕЕ: видит оба источника и разрешает спор в
    // пользу более позднего волеизъявления владельца, не синтезируя ничего своего.
    rel: 'docs/tasks/morning-gates-state.json',
    required: false,
    label: 'СОСТОЯНИЕ ГЕЙТОВ УТРА — `magistral` несёт выбор владельца, сделанный ПОЗЖЕ assertions; при расхождении см. правило магистрали',
  },
  {
...
export async function runMainDayIssue(options) {
  ...
  const sections = [
    // Зверь B1, кузен «анонс вместо работы»: 28.07 Grok четырежды ответил одной строкой
    // «Собираю MAIN_DAY_ISSUE…». Требование формы обязано стоять В ГОЛОВЕ сборки.
    'ТВОЙ ОТВЕТ — САМ ДОКУМЕНТ MAIN_DAY_ISSUE, целиком, в markdown. Первая строка ответа — заголовок «# …». Обязательны все пять секций строго заголовками второго уровня: «## Магистраль», «## Подкрепление», «## Перспективные», «## Экспериментальные», «## Санитарные» (нечем наполнить — заголовок и «— пусто —» под ним). ЗАПРЕЩЁН ответ-анонс («Формирую…», «Собираю…», описание намерений) — гейт скелета бракует его как полый.',
    '',
    'Ты — Teamlead (Tarasov) виртуальной команды Membrana.',
    'На основе стендапа, ревью, плана и реестра задач зафиксируй **один** обязательный фокус дня в MAIN_DAY_ISSUE.',
    '**МАГИСТРАЛЬ БЕРЁТСЯ ИЗ ВОЛЕИЗЪЯВЛЕНИЯ ВЛАДЕЛЬЦА, НЕ СИНТЕЗИРУЕТСЯ.** Если `docs/tasks/main-day-assertions.json` содержит `sources[]` — магистраль дня есть `sources[0].claim` (это прямой выбор/слово владельца с указателем). Синтезировать свою магистраль из входов ЗАПРЕЩЕНО, пока owner-source задан: это ровно ошибка, против которой построен probe (16.07 план назначил фантом; 18.07 генератор чуть не выбрал #598/#599 вопреки владельческому «подключить генератор»). Входы (стендап/ревью/горизонт/граф) — КОНТЕКСТ для раскрытия магистрали, а не источник её выбора. Только если `sources` пуст — синтезируй и пометь honestly «магистраль не задана владельцем». **РАЗРЕШЕНИЕ РАСХОЖДЕНИЯ (У1, 31.07):** если `docs/tasks/morning-gates-state.json` несёт `magistral` и его `day` — СЕГОДНЯШНИЙ, то это волеизъявление владельца ПОЗДНЕЕ, чем `sources[0]`, и магистраль дня есть выбор гейта. Расхождение НЕ замалчивать: назвать его прямо в таблице обоснования отдельной строкой — «магистраль взята с гейта, assertions не перечеканены» — потому что расхождение само есть находка (перечеканка main-day-assertions.json каноном предписана и не сделана). Синтезировать при этом по-прежнему запрещено: оба источника владельческие, спор решается свежестью, а не догадкой.',
  '',
    '---',
