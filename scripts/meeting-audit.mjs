...
function main() {
  ...
  const { topics, protocols, checks, violations, corpusEmpty } = runMeetingAudit(repoRoot, id);
  ...
  // Итог при пустом корпусе НЕ говорит «нарушений 0»: поцикловые проверки (2, 4, 6) не
  // получили ни одного предмета, и прежняя формулировка читалась как чистота. Ратифицированный
  // вердикт M5 (30.07): «чисто» при пустом корпусе запрещено — это «корпуса нет».
  if (corpusEmpty) {
    console.log(
      `\nКОРПУСА НЕТ: протоколов 0 при ${topics.length} повестке(ах). ` +
        'Проверки 2, 4 и 6 не выполнялись ни на одном предмете — это НЕ «нарушений 0». ' +
        `Структурных нарушений в том, что проверить удалось: ${violations}.`,
    );
  } else {
    console.log(
