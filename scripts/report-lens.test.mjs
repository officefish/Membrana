...
// 28.07: норма ужесточена проводом линзы «живые ссылки» — голый #N больше не
// проходит (в телеграме это текст, не ссылка); законная форма — [#N](url).
test('swallow-mirror: чистый черновик проходит, детали кликабельными ссылками', async () => {
  const { checkSwallowDraft } = await import('./lib/swallow-mirror.mjs');
  const draft = ['☀️ Доброе утро! Сегодня чиним надёжность утреннего планирования.', '',
    ...
    'Гигиена: 🧹 без изменений.', '',
    'Детали: [#592](https://github.com/officefish/Membrana/issues/592) · [#788](https://github.com/officefish/Membrana/pull/788)'].join('\n');
