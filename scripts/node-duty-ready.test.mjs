...
test('ПОРЧА: гибернация включена — предикат красный с причиной', () => {
  const bad = judgeSleep({ standby: PC_EN_0, hibernate: PC_EN_20M });
  assert.equal(bad.ok, false);
  assert.match(bad.detail, /гибернация через 1200 с/u);
  assert.match(bad.fix, /powercfg \/h off/u);
});
