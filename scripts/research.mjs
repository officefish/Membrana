...
async function main() {
  ...
  // Ненулевой код при подозрительных секциях: 15.07 «выжимка по теме» была заявлена
  // по прочтении ОДНОГО ответа из трёх, а Q2 оказался про мембранную ткань. Пусть
  // о таком сообщает код возврата, а не добросовестность читающего.
  if (result.suspect?.length > 0) {
    console.error(
      `\n⚠ Поиск, похоже, не нашёл тему: ${result.suspect.join(', ')}.` +
        '\n  Секции помечены в артефакте. Переформулируй без внутренних имён проекта и прогони снова.',
    );
    process.exitCode = 1;
  }
}
