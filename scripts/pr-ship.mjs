 ...
 *   2. состояние ствола печаталось, но не доезжало до решения: `printFinalPrState`
 *      ничего не возвращал, и судить было не по чему;
 ...
 * в другую сторону.
 *
 * ЧЕГО ЭТА ФУНКЦИЯ НЕ ЗНАЕТ (названо ревью #2248, чтобы комментарий не обещал лишнего):
 * шаги, ПРОПУЩЕННЫЕ по `guard` (например, ff-sync, когда base держит соседнее дерево), в
 * `failedSteps` не попадают. Это осознанно: пропуск по guard — не падение, а отказ от
 * невозможного действия, и цель он не отменяет. Но если однажды guard начнёт скрывать
 * настоящую неудачу, эта функция об этом не узнает — сюда придётся вернуться.
 *
 * @param {{
 *   executed: boolean,
