...
test('один вопрос — повестка годна', () => {
  // Контракт расширен 18.07: повестка обязана ТРЕБОВАТЬ список посылок (добор задним
  // числом провалился на M2″ — комната назвала посылки чужого вердикта).
  const topic = '## Вопрос\n\n**A1 — свести граф с ADR или разделить?**\n\nПолный список посылок обязателен.\n';
  assert.equal(meetingAgendaProblem(topic), '');
});
