...
description: "Bridge an adopted Membrana insight into a running sprint: gate check, transcribe the review's «Следующий шаг» into a task-prompt, register in docs/tasks/registry.json with an insightId back-link, and set a cold-session entry point. Use when user says перейти из инсайта в спринт, довести инсайт до спринта, adopted insight → task, оформить task-промпт по инсайту, or start building an adopted insight. Do NOT use for creating the insight itself (membrana-insight) or archiving a finished task (membrana-task-lifecycle)."
---
