...
description: "Экспериментальные запросы к LLM через proxy (OpenRouter, FreeModel) — те же mjs-паттерны, что anthropic:smoke/task, но ключи в .env.llm-proxy. Use when user asks opencode:ask, opencode:task, llm-proxy, OpenRouter proxy. Do NOT use for ritual:day, code-review, anthropic:smoke."
---
