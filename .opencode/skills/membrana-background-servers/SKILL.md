...
description: "Routes Membrana work to the correct background-* NestJS package: office (integrations), media (blobs, trends), cabinet (auth, membranes). Use when adding Claude/Linear, WAV storage, sample library API, pairing, or choosing which background server to extend. Do NOT use for packages/services/* analyzer services or client-only IndexedDB fallback."
---
