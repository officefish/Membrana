...
4. **Живые ссылки** — отдельно от линзы. КАНОН ФОРМЫ (28.07, слово владельца после
   двух неудач подряд): единственная законная запись ссылки в письме —
   **markdown `[#N](https://github.com/officefish/Membrana/pull|issues/N)`**.
   - голый код `#N` — в телеграме ПРОСТО ТЕКСТ, не ссылка (28.07 партнёрам ушли мёртвые детали);
   - голый URL — тоже не годится (27.07 пришёл статикой);
   - office умеет: `mdToTelegramHtml` даёт `<a href>` (код с 14.07, прод его несёт —
     диагноз «прод отстал» был неверным, виноват формат письма).
   Гейт держит норму машинно: для дневной ласточки `yarn morning:gate swallow --draft`,
   для вечерней `yarn evening:gate partner-swallow --draft`. Оба отказывают на
   голых `#N` и числах-без-адреса и печатают готовую форму исправления.
