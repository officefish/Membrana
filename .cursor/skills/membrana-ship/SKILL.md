...
0. **`yarn branch:check`** — держатель и грамматика ветки (Р4, вердикт
   `m4-grammar-manual`): шапка «тип · держатель (+источник) · заморозка».
   Красный исход (`CONFLICT` / `MISSING` / night-заморозка / вне словаря /
   ref-коллизия) — **возврат на доработку** (T8): поправить имя/держателя
   (`--holder <персона>` — явное слово) и повторить, не продавливать.
