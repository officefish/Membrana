...
| [`membrana-tasks-workshop`](./membrana-tasks-workshop/SKILL.md) | инвентарь primary-мастерской `docs/tasks`: `yarn task:tools` → таблица + `--doc`; кит `kits/tasks-master`; ≠ audit/decompose; mirrored Claude/Agents/OpenCode |
| [`membrana-pr-audit`](./membrana-pr-audit/SKILL.md) | ревизия открытых PR: параллельные read-only аудиторы → вердикты merge-ready/close-superseded/low-value/needs-work со свидетельством-в-main; устаревшие → `pr:recreate`; мердж проверять `pr:verify`; mirrored to Claude |
| [`membrana-containerization-master`](./membrana-containerization-master/SKILL.md) | cold-start крафта контейнеров/китов: 2 паттерна + `kits/containerization-master` + процедура `containerization`; не путать с `procedure-frames` (#900); mirrored to Claude |
...
| [`membrana-branch-salvage`](./membrana-branch-salvage/SKILL.md) | исполнение ратифицированного exact-tip плана: reconcile → одна ref-мутация → ADR-0020 post-check → fail-closed closeout; без semantic verdict и удаления worktree; mirrored to Claude/Agents |
...
| [`membrana-hackathon`](./membrana-hackathon/SKILL.md) | хакатон, hackathon, H1-H4 relay — строгая эстафета четырёх передач с `stage-completion-checklist`, финальным аудитом и возвратом к ритму; mirrored to Claude / Codex agents / Opencode |
