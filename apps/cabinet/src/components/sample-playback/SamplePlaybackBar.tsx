...
export function SamplePlaybackBar({
  playback,
  compact = false,
  onExport,
  showExport = false,
}: SamplePlaybackBarProps) {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <button
          type="button"
          className={`btn btn-circle ${compact ? 'btn-xs' : 'btn-sm'} btn-primary shrink-0`}
        ...
        {showExport && onExport ? (
          <button
            type="button"
            className={`btn btn-ghost shrink-0 ${compact ? 'btn-xs' : 'btn-sm'}`}
            disabled={!hasSelection || isLoading}
