...
export const QualityWidget: React.FC<QualityWidgetProps> = React.memo(
    ...
    return (
      <div className="rounded-2xl bg-base-200 border border-base-300 p-4">
        ...
        <div className="mt-1 grid grid-cols-2 gap-2 text-center">
          ...
          <div>
            <p className="text-xs text-base-content/50">Шум</p>
            <p className="text-sm font-mono font-bold text-base-content/80">
              {noise === 0 ? '—' : noise}
            </p>
          </div>
...
export const SpectrumWidget: React.FC<SpectrumWidgetProps> = React.memo(({ spectrumData }) => {
  ...
  return (
    <div className="rounded-2xl bg-base-200 border border-base-300 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs text-base-content/40 font-mono">Гц</span>
      </div>
      ...
      <div className="flex items-center gap-4 mt-3 justify-center">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-base-content/50">Низ</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-cyan-400" />
          <span className="text-xs text-base-content/50">Середина</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2 h-2 rounded-full bg-indigo-400" />
          <span className="text-xs text-base-content/50">Верх</span>
        </div>
      </div>
    </div>
  );
...
export const VolumeWidget: React.FC<VolumeWidgetProps> = React.memo(({ volume }) => {
...
export interface WaveformWidgetProps {
  waveformData: number[];
  gradientId: string;
  filterId: string;
}

export const WaveformWidget: React.FC<WaveformWidgetProps> = React.memo(
  ({ waveformData, gradientId, filterId }) => {
...
export type WidgetToggleKey = keyof Pick<
  MicStreamVizPluginConfig,
  'showVolume' | 'showQuality' | 'showWaveform' | 'showSpectrum'
>;
...
export interface WidgetControlsProps {
  widgetStates: Pick<
    ...
    'showVolume' | 'showQuality' | 'showWaveform' | 'showSpectrum'
  >;
  onToggle: (widget: WidgetToggleKey) => void;
}

export const WidgetControls: React.FC<WidgetControlsProps> = React.memo(
  ({ widgetStates, onToggle }) => {
    const widgets: { id: WidgetToggleKey; label: string }[] = [
      { id: 'showVolume', label: 'Индикатор громкости' },
      { id: 'showQuality', label: 'Качество / SNR' },
      { id: 'showWaveform', label: 'Осциллограмма' },
      { id: 'showSpectrum', label: 'Спектр' },
    ];
