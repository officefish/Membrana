          ...
          {rows.map((row) => (
            <tr key={row.field}>
              ...
              <td className="font-mono text-xs tabular-nums">
                {row.field.includes('Trend') ||
                row.field === 'longTermStability' ||
                row.field === 'periodicity' ||
                row.field === 'envelopeShape'
                  ? localizeValue(row.actual)
                  : row.actual}
              </td>
    ...
    <div className="flex flex-col gap-3 min-h-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div>
            ...
            <div className="text-xs text-base-content/60">
              Общее совпадение {breakdown.overallScore}% · спектр{' '}
              {breakdown.spectralScore}% · время {breakdown.temporalScore}%
            </div>
          </div>
        </div>
      ) : null}
      ...
      <div className="space-y-4">
        {showSpectral
          ? section === 'all'
            ? renderSection(
                'Спектральные метрики (30% веса)',
                spectralRows,
                breakdown.spectralScore,
              )
