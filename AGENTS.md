...
- **Env in worktrees**: agent scripts that import `scripts/_anthropic-env.mjs` load secrets from `MEMBRANA_ENV_PATH` first, then search upward for the nearest `.env`. This lets isolated worktrees under `.worktrees/` reuse the root `.env` for Anthropic/OpenAI/RAG rituals without copying secrets. Do not duplicate or commit `.env` files inside worktrees.
- **Headroom metrics are per-client:** when reporting Headroom, separate measured traffic (`client=codex`, `client=claude-code`, etc.) from operational work outside Headroom. Use [`docs/HEADROOM_AGENT_TELEMETRY.md`](docs/HEADROOM_AGENT_TELEMETRY.md) and `yarn headroom:agent-report`.
