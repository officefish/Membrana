...
#### Порядок обращения к инструменту — норма

Источник истины — здесь. Зеркала в [`.cursorrules`](.cursorrules) и
[`.claude/CLAUDE.md`](.claude/CLAUDE.md) — **строки-указатели, не вторая редакция**; скиллы и
промпты ссылаются, а не копируют. Пол холодной сессии несёт эту строку **только проекцией** —
ручная запись в выдачу запрещена (§6 контракта `workshop-wires`).
...
3. Не нашлось ни обзором, ни глаголом — тогда разведка: сначала `codebase-memory-mcp` (граф
   символов), затем греп по тексту. Оба — законный ход, а не провал.
4. Поиск по коду текущей задачи при уже известном инструменте **не запрещён**. Абсолютного
   запрета на греп нет.

> **`codebase-memory-mcp` — разведка, а не прибор** ([`PRIORITY_HOOK_CANON`](docs/meeting/workshop-wires/PRIORITY_HOOK_CANON.md),
> решено владельцем 31.07). Хук `SessionStart` требует его «первым для любой разведки кода»;
> это правило второго уровня и остаётся в силе **внутри своего класса** — граф впереди текста,
> потому что точнее. Но обращение к мастерской разведкой не является: глагол **знает** ответ о
> состоянии дома, а MCP и греп его **ищут**. Различение по роду ответа, а не по инструменту,
> поэтому очередь между ними — не конкуренция.
>
> Коллизия была живой: до 31.07 приоритет не записан нигде, и холодная сессия решала его на
> ходу. Решала верно — но следующая могла бы иначе, и обе были бы правы по букве.

> **Это норма-порядок, а не снимок инвентаря.** Запрет держать перечень команд руками
> (врезка выше) остаётся в силе и этой нормой не отменяется: порядок обращения — правило,
> список инструментов — генерируемый факт. Смешать их значит вернуть тот самый рукописный
> снимок под новым заголовком.
>
> **Почему не в таблице граблей.** Грабли — про то, что врёт или кусает; это устав порядка,
> он не кусает, а предписывает.
>
> **Почему без зуба.** Невызов инструмента машине в момент нарушения не виден: гейт мог бы
> ловить только вызовы, а нарушение — это их отсутствие. Подпорка — разбор транскриптов
> постфактум с находкой, а не блоком (§8 контракта `workshop-wires`). Признак жизни нормы
> измерим: за 28 дней доля сессий, начатых разведочным поиском, не выше 0.2, и доля сессий
> хотя бы с одним вызовом мастерской не ниже 0.5; меньше пяти сессий в окне — честное
> «выборки недостаточно», а не зелёный. **Плакат «строка есть в `AGENTS.md`» признаком
> жизни не является.**
...
| `pr:ship` при CONFLICTING/DIRTY | **STOP до merge** (ATF4-1). Ребейз — да (`yarn git:rebase-continue` при конфликтах), но **force-push запрещён политикой** (deny-лист рядом с прод-деплоем и `reset --hard`), поэтому уже отправленную ветку не перезаписать. Работающий маршрут: новая ветка от свежего base → `yarn pr:ship --no-commit …` → закрыть перекрытый PR ссылкой на новый. 30.07 это стоило лишнего PR (#1487 вместо #1484), пока грабля советовала недоступный force-with-lease (#1493 Ф3) |
| Raw `git worktree remove` (тем более циклом) | **ЗАПРЕЩЁН. Снос деревьев — только `yarn repo:clean --execute --worktrees`**, форма нормы та же, что у мерджа (`pr:ship`, #700). Снос — контролируемый снос из шести кадров (ADR-0020): снимок живых деревьев → снятие связей наружу → снятие с учёта → проверка, что каталог ушёл → **пост-чек живых деревьев** → разрешение следующего. Raw-команда не делает ни снятия связей, ни пост-чека: рекурсивное удаление идёт СКВОЗЬ junction `node_modules/@membrana/*` в `packages/*` соседа. Цена класса: 20.07 — 2019 файлов, 29.07 — 2097 + 2089, **06.08 — 2152** (третий раз, единственный мимо инструмента). Дерево ВНУТРИ другого дерева (`.claude/worktrees/…`, `.worktrees/…`) — отдельная ловушка: вложенность не печатает никто, проверять до сноса |
...
| `\w` / `\b` в JS-регулярках — ASCII даже с флагом `u` | Фильтр русских заметок `\b`-шаблоном молча вернул пусто (26.07): `[А-Яа-яЁё]` в `\w` не входит, границы слов на кириллице не работают. Лечение: явные `[\p{L}\p{N}]` с `u` или явная кириллица. Соседка строки ниже — одна лемма «латиница vs кириллица в текстовых сверках» |
| Сверка латинских id/slug с русским склоняемым текстом «по вхождению» | Прогон сверки заметок 26.07: **39 ложных против 8 настоящих**. Лечение: словарь соответствий или транслит, не `includes` по подстроке |
| «Голый `fetch` в `deepseek.service` — грабля» | **Ложная находка, снята 29.07.** Прямой путь DeepSeek ратифицирован [ADR-0007](docs/adr/ADR-0007-night-narrative-provider-chain.md): ценность канала — независимость от прокси. «Починка» сломает решение. Метка `network-tooth:allow-bare-fetch ADR-0007` — освобождение, а не долг; `network:tooth` печатает такие каналы отдельной строкой |
| `Cannot find module '@membrana/*'` | Ошибка врёт про место: указывает на потребителя, а причина — ссылка ведёт в дерево, где пакет не собран. `yarn workspace:links` называет её одной строкой. **`turbo` не лечит**: отдаёт `cache hit` и собирает СВОЮ копию, а ссылка смотрит в чужую (#1465 Ф1) |
| `yarn npm audit` руками, без `--all` | **Врёт в успокаивающую сторону:** смотрит один корневой воркспейс. 30.07 дал **3** находки вместо 23 — и на этом основании корзина #1422 была объявлена устаревшей. Правильный вызов один: `yarn deps:watch --mode morning` (внутри `--all --recursive`, `scripts/deps-watch.mjs:41`) |
| `yarn install` в junction-дереве всегда `Failed with errors` | Падают **нативные сборки** — `esbuild@0.27.7` (`Manifest not found`) и `cpu-features` (опциональный ускоритель ssh2). `yarn.lock` при этом обновляется корректно. Смотреть, ЧТО упало, а не на итоговую строку: 30.07 четыре установки подряд «падали» и все четыре сработали (#1493 Ф1) |
| Машинный аудит на пустом корпусе | «Нарушений: 0» на нуле предметов — **ложная зелёнка, а не чистота.** Поцикловые проверки не дают строк, итог читается как «проверено и чисто». Класс рецидивный: #696 (маска повесток) → 30.07 (маска протоколов: `meeting:audit` не видел ни одного из 10 протоколов `sprint-honest-performers`, потому что искал префикс `<id>-`, а комнаты пишут `sprint-honest-m0-…`). Лечение общее: корпус **объявляет контейнер** (ссылки в `MEETING_ACTIVE.md`), догадка по имени — только добор; при пустом корпусе печатать «КОРПУСА НЕТ». Ратифицированный M5: «чисто» при пустом корпусе запрещено |
| Порядок артефактов ≠ алфавит имён | Ратифицированный владельцем порядок комнат может быть непоследовательным (`… 6 → 8 → 7 → 9` у `sprint-honest-performers`) — тогда M8 честно датирован раньше M7. Проверка, сортирующая по именам, объявит это «протокол раньше предшественника». Порядок прогона брать из объявления контейнера, а не выводить из имён |
| `node --test <каталог>` на Windows | Даёт `pass 0, fail 1` — читается как падение своего теста. Перечислять файлы явно; общий прогон — `node scripts/test-scripts-run.mjs --group <g>` |
| kind ветки ≠ тип коммита | Словари разные **намеренно**: kind (`layer-rules.json`) описывает характер работы (`tooling`, `meeting`, `storm`, `night`, `sprint`), тип коммита — conventional. `pr:ship --type tooling` отказывает сразу и называет замену (#1465) |
| `pr:ship` встал молча после «ревью не найдено» | Гейт публиковал `review/teamlead=pending`, и следующий `ci-wait` ждал статус, который ставит шаг ЗА ним — самоссылка, 15 минут тишины (PR #1461). Снято 29.07; связку gate→ревью→merge делает `pr:ship --with-review` одним проходом, BLOCK по-прежнему стоп |
...
| Утро требует чекаут `main` | **Нет (#1232):** нужно `freshEnough ∧ clean` vs `origin/main` (порог behind=0). `main` никому не выдаётся — держатель = находка. Грязь/чужие правки → эскалация владельцу, не checkout. Коммит/push с `HEAD == main` режет хук (`ALLOW_MAIN_COMMIT=1`) |
| Новый `scripts/_ssh-*.mjs` | Под gitignore — только `git add -f`, иначе молча не войдёт в коммит (#476 п.7) |
| Секрет-образец в тесте **литералом** | Собирать **в рантайме** из частей (образец — `scripts/secret-redact.test.mjs`). Литерал ловится CI-скатом и, пока ветка жива в origin, красит заявки СОСЕДНИХ сессий: скан идёт по истории всех ветвей (26.07, #1262). `.gitleaksignore` привязан к SHA — при ребейзе перестаёт совпадать, для новых фикстур не годится |
| «Локально gitleaks чисто» | Не гарантирует CI: **версии разные** (локально может быть 8.30.1, в CI пришпилен 8.21.2 — правила не совпадают, 8.30.1 пропустил PEM-фикстуру). `pre-push [#1262]` называет расхождение вслух |
| Правка файла через `python`/heredoc | **`python` в среде агента не работает** — правка молча не применяется, тесты идут на СТАРОМ файле и дают ложный зелёный (26.07). Только редактором |
| Фоновый bash без явного `timeout` | Умирает по дефолтным 2 мин **видом успеха** (completed, exit 0, пустой лог — 27.07 трижды). Явный timeout обязателен; «completed + пустой вывод» = вероятный обрыв, проверять состоянием. Ждать мердж — `yarn pr:verify <N> --wait`, не самодельный опросчик |
| `pr:ship` через `Start-Process` / фон / внешний timeout | **Родительская оболочка может вернуть 0, пока дочерний `pr:ship` падает.** Вещдок 22.08: прямые `node scripts/pr-ship.mjs` и `yarn pr:ship` на `push`/`ci-wait` вернули 1; `Start-Process` без `-Wait` вернул 0 родителю и отпустил доклад раньше дочернего падения. Лечение: запускать foreground; если нужен `Start-Process`, только `-Wait -PassThru` и затем `exit $p.ExitCode`. Для merge доклада всё равно проверять состояние: `yarn pr:verify <N>` / `gh pr view <N> --json state,mergeCommit`, не exit родительской обёртки. |
| Foreground `sleep` в агентском шелле | **Заблокирован средой целиком**, не «работает медленно»: пауза-ожидание строится только на инструментных ожидателях (`pr:wait`, `--wait`-флаги) или фоновом запуске с уведомлением — самодельный `sleep N && …` не исполнится (#1265, соседка строки выше: та — про фон, эта — про передний план) |
| Состояния issue/PR поштучным `view` в цикле | **Списком**: `yarn task:states <N…>` / `fetchStatesBatch` — один вызов, явный таймаут, honest unknown на сеть (#1322; 26.07 тринадцать `view` встали на 5-мин таймауте) |
| Ревью ветки в `docs/discussions/` | `.gitignore:149` `branch-*-code-review.md` — не коммитится. `git add` откажет; правило видно `git check-ignore -v` |
...
| `yarn pr:ship … \| tail` | **Пайп маскирует exit-код** (код пайплайна = код `tail` = 0): ship упал (push-500 / pre-commit / pr-wait), а видно «exit 0». НЕ гнать ship через `\| tail`; факт мерджа проверять СОСТОЯНИЕМ — `yarn pr:verify [N] [--file <path>]` (ассерт `MERGED ∧ mergeCommit`), а не exit-кодом (#1166). `pr:ship` теперь сам ассертит мердж шагом `verify` |
| Устаревший PR (сотни коммитов позади) | Force-push закрыт политикой → **не ребейзить**, пересобирать: `yarn pr:recreate <N> [--drop <path>]` — net-дельта на свежую ветку от main, пофайлово (0 main-коммитов после базы → берём версию ветки; иначе MANUAL 3-way). НЕ `git merge origin/main` в отставшую ветку: стейджит весь catch-up и ловит **ложный gitleaks** `protect --staged` (контент уже в main) — реальная проверка `gitleaks detect --log-opts=<range>` (#1167) |
| Ветку основал на squash-merged ветке | PR выходит **CONFLICTING** (контент задвоился: squash положил его в main без коммита-предка). Новую ветку брать от `origin/main`, не от только что смёрженной; если уже основал — `git cherry-pick <свой-коммит>` на ветку от main (#1176) |
| pre-push `vite: command not found (127)` на docs-правке | **ПОЧИНЕНО #1168**: markdown внутри пакета (`packages/*/*.md`) больше НЕ поднимает билд-цепочку пакета — `scripts/prepush-typecheck-scope.mjs` исключает `.md/.mdx` из affected. Docs-only push → typecheck skip; корневой `package.json`/scripts → skip (не глобальная зависимость). При рецидиве vite-127 — проверь, не вернулся ли инлайн `--filter='...[origin/main]'` в `.githooks/pre-push` |
...
| Ветка «от текущей точки» при занятом дереве | **Утаскивает чужие коммиты (26.07, #1272).** Встать на свежий `origin/main` не дало занятое дерево → ветвление на месте → в базе оказался чужой невлитый коммит, заявка предлагала чужой труд под моим именем. Коммиты при этом были поимённые и честные — ложь на уровне **базы**. Ловит `yarn branch:check-base` (exit 2, называет автора). Выход при занятом дереве — **отдельное рабочее дерево**, не ветвление на месте |
| Локальный секрет-скан «тормозит» | **Не он.** Хук гоняет `gitleaks protect --staged` — только подготовленные файлы, сотни миллисекунд. Таймауты 26.07 давали составные команды (коммит + push + проверки одной строкой) и `pre-push` (typecheck/wire-sync). Не «оптимизировать» хук — разносить команды |
| Скан секретов красный «не по вашей вине» | В проверках ветки скан идёт **по диапазону заявки** (`base..HEAD`, #1272). Полная история — только на общей ветке и по расписанию. Если красный всё же чужой — не заводить исключения в `.gitleaksignore`, а убирать повод (26.07: сосед пересобрал PEM-фикстуру в рантайме — правило не ослаблено) |
| `pre-push` в свежем worktree | Без `node_modules` хук честно объявляет пропуск yarn-проверок и **продолжает** те, что на голом node (#1272). Не глушить весь хук через `SKIP_PREPUSH` — это отключает и гейт ответственности |
| Повторный `task:register` ради `--linear` | **Больше не обнуляет** `leadPersona` / `supportPersonas` / `notes` (#1272). До 26.07 затирал их пустыми, и `pre-push trace-gate` вставал с «ответственный не назначен». Снятие ответственного — осознанный акт, не побочный эффект линковки |
| Тело комментария в `gh` через двойные кавычки | Оболочка **выполняет** обратные кавычки внутри текста — комментарий уходит покалеченным (26.07, дважды). Только `--body-file` с heredoc `<<'EOF'` |
...
Слот **отчётов** в плоскости `docs/audit/` (report-plane): здесь снимки и разборы
**про** задания. Сами карточки — предметный дом [`docs/tasks/`](docs/tasks/)
(`registry.json`). Не путать с `docs/tasks`. Аудит — по образцу git-контейнера;
снимки не класть вне слота. Канон:
